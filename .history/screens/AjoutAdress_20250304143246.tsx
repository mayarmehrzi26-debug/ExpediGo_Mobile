...
const AjoutAdress: React.FC<AjoutAdressProps> = ({ navigation }) => {
  const [zone, setZone] = useState("");  // Affiche la zone (lieu, ville)
  ...
  // Fonction pour récupérer le nom de la place et la ville
  const getCityFromCoords = async (latitude: number, longitude: number) => {
    try {
      const [address] = await Location.reverseGeocodeAsync({ latitude, longitude });
      if (address) {
        const place = address.name || address.street || "Place inconnue";  // Nom de la rue ou place
        const city = address.city || address.region || "Ville inconnue"; // Ville ou région
        return `${place}, ${city}`;
      }
    } catch (error) {
  ...
  const handleMapPress = async (event: any) => {
    ...
    const zoneName = await getCityFromCoords(latitude, longitude);  // Récupère la zone au format "place, ville"
    setZone(zoneName);  // Met à jour la zone dans le champ "zone"
      ...
      <Modal animationType="slide" transparent={false} visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalContainer}>
          ...
          <MapView
            ...
            initialRegion={{
              latitude: 33.8869, // Latitude approximative de la Tunisie
              longitude: 9.5375, // Longitude approximative de la Tunisie
              latitudeDelta: 0.1, // Valeur pour zoomer (zoom initial)
              longitudeDelta: 0.1, // Valeur pour zoomer (zoom initial)
            }}
            onPress={handleMapPress}
            showsUserLocation={true} // Affiche la localisation actuelle de l'utilisateur
            zoomEnabled={true} // Permet de zoomer
            scrollEnabled={true} // Permet de faire défiler la carte
            rotateEnabled={false} // Désactive la rotation de la carte
          >
