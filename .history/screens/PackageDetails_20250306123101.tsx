...
const PackageDetails: React.FC<PackageDetailsProps> = ({ route }) => {
  ...
  useEffect(() => {
    const fetchPackageAndDeliveryDetails = async () => {
      try {
        const packageRef = doc(firebasestore, 'packages', scannedData);
        const packageSnap = getDoc(packageRef);
        
        const packageData = (await packageSnap).data() as Package | undefined;
        if (packageData) {
          setPackageDetails(packageData);
          const deliveryRef = doc(firebasestore, 'livraisons', packageData.deliveryId);
          const deliverySnap = getDoc(deliveryRef);
          
          const deliveryData = (await deliverySnap).data() as Delivery | undefined;
          if (deliveryData) {
            setDeliveryDetails(deliveryData);
          }
        }
