...
const AddTicket: React.FC = ({ route }) => {
  const { refreshTickets } = route.params || {}; // Get refreshTickets from params
  ...
  // Fonction pour générer un ID numérique
  const generateNumericId = async () => {
    ...
    if (counterDoc.exists()) {
      ...
      await updateDoc(counterRef, { lastId: newId }); // Mettre à jour le compteur
      return newId;
    } else {
      await setDoc(counterRef, { lastId: 1 }); // Initialiser le compteur
      return 1;
    }
  };
  ...
  const handleAddTicket = async () => {
    ...
    const numericId = await generateNumericId(); // Générer un ID numérique

    const ticketData = {
      id: numericId, // Ajouter l'ID numérique
      type: selectedType,
    ...
    try {
      ...
      if (refreshTickets) {
        refreshTickets(); // Rafraîchir la liste des tickets
      }
