...
export const NouvelleLivraison: React.FC = () => {
  ...
  const [activeScreen, setActiveScreen] = useState("NouvelleLivraison");
...
const generateQRCode = (deliveryId) => {
  // Générer l'URL du code QR en utilisant l'ID de la livraison
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?data=${deliveryId}&size=200x200`;
  ...
  const saveDelivery = async () => {
    ...
    try {
      // Récupérer le premier statut
      const statusSnapshot = await getDocs(collection(firebasestore, "Status"));
      const defaultStatus = statusSnapshot.docs[0]?.id; // Récupérer l'ID du premier statut
    const deliveryData = {
 ...
 try {
    ...
    const deliveryRef = await addDoc(collection(firebasestore, "livraisons"), deliveryData);
    
    // Générer le code QR avec l'ID de la livraison
    const qrCodeUrl = generateQRCode(deliveryRef.id); // Supposons que generateQRCode génère l'URL du code QR
  ...
  <CustomDropdown
    ...
    onSelect={(value) => {
      if (value === "new_product") {
        navigation.navigate("AjoutProd" as never); // Navigation vers l'écran d'ajout de produit
      } else {
        setSelectedProduct(value); // Mettre à jour l'état avec la valeur sélectionnée
      }
    }}
    selectedValue={selectedProduct} // Passer la valeur sélectionnée
  />
</View>
