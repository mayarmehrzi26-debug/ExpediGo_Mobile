...
const AjoutAdress: React.FC<AjoutAdressProps> = ({ navigation }) => {
  ...
  const handleMapPress = async (event: any) => {
    ...
    // Obtenez les zones basées sur la nouvelle position
    const zones = await fetchZones(newLocation); // Fonction pour obtenir les zones
    setSuggestions(zones);
    setModalVisible(false); // Ferme la modal après sélection
  };

  const fetchZones = async (coordinate: { latitude: number; longitude: number }) => {
    // Remplacez ceci par votre logique pour obtenir les zones
    return [
      "Bab Bhar, Sousse",
      "Bab Alioua, Tunis",
      "Bab Saadoune, Tunis",
      // Ajoutez d'autres zones selon les coordonnées
    ];
  };
            ...
            {suggestions.length > 0 && (
              <View style={styles.suggestionsContainer}>
                {suggestions.map((suggestion, index) => (
                  <TouchableOpacity key={index} onPress={() => setZone(suggestion)} style={styles.suggestionItem}>
...
const styles = StyleSheet.create({
  ...
  suggestionsContainer: { backgroundColor: "white", borderRadius: 8, padding: 10, marginTop: 5 },
