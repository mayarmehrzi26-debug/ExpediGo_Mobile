...
const saveDelivery = async () => {
  ...
  try {
    ...
    // Récupérer l'ID numérique pour le package
    const packageCounterDocRef = doc(collection(firebasestore, "counters"), "packageCounter");
    const packageCounterDoc = await getDoc(packageCounterDocRef);
    let newPackageId = 1; // Valeur par défaut

    if (packageCounterDoc.exists()) {
      newPackageId = packageCounterDoc.data().count + 1; // Incrémentez le compteur
      await updateDoc(packageCounterDocRef, { count: newPackageId }); // Mettez à jour le compteur
    } else {
      await setDoc(packageCounterDocRef, { count: newPackageId }); // Créez le document si nécessaire
    }
    ...
    await setDoc(doc(firebasestore, "packages", newPackageId.toString()), {
      deliveryId: newId,
      qrCodeUrl,
    });
