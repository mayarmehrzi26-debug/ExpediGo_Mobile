...
const LOCATIONIQ_API_KEY = 'pk.17568ced89faf24c4ee34b7efef1d10a'; // Remplacez par votre clé
...
const TrackingScreen = () => {
  ...
  const calculateRegion = (coordinates: Coordinate[]) => {
    ...
    const padding = 0.01; // Padding supplémentaire autour du trajet
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // Géocodage des deux adresses
        const [from, to] = await Promise.all([
          geocodeWithLocationIQ(fromAddress),
          geocodeWithLocationIQ(toAddress)
        ]);
  ...
  const routeRegion = calculateRegion([fromCoords, ...coordinates, toCoords]);
  if (loading) {
    return (
  ...
  return (
    <View style={styles.container}>
      ...
      <MapView
    style={styles.map}
    region={routeRegion} // Utilisez 'region' au lieu de 'initialRegion'
    onMapReady={() => {
      // Zoom supplémentaire après le chargement si nécessaire
  ...
  >
      ...
      <View style={styles.footer}>
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>Trace approximative - ligne droite</Text>
        </View>
      </View>
    </View>
  );
