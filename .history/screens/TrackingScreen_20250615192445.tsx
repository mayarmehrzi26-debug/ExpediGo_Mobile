...
const TrackingScreen = () => {
  ...
  // Fonction améliorée de géocodage avec gestion des erreurs
  const geocodeAddress = async (address: string): Promise<Coordinate> => {
    try {
      ...
      const response = await fetch(
        `https://api.locationiq.com/v1/autocomplete?key=${LOCATIONIQ_API_KEY}&q=${encodeURIComponent(address)}&limit=1&format=json`
      );
  ...
  // Tentative de géocodage avec réessai
  const geocodeWithRetry = async (address: string, retries = 2): Promise<Coordinate> => {
    try {
  ...
  // Suivre la position du livreur en temps réel
  useEffect(() => {
      ...
      if (doc.exists()) {
        ...
        if (data.commandeId === deliveryId && 
          ...
          if (mapRef.current && fromCoords && toCoords) {
            const allCoordinates = [fromCoords, toCoords, newLocation];
