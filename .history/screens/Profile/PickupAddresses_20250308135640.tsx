...
const PickupAddresses: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(firebasestore, "adresses"), (snapshot) => {
      const fetchedAddresses = snapshot.docs.map((doc) => ({
        ...doc.data(),
        docId: doc.id, // Ajouter l'ID Firestore si besoin
      }));
  ...
  // Ajouter une adresse avec ID numérique
  const addAddress = async () => {
    const counterRef = doc(firebasestore, "counters", "adresses");

    try {
      // Récupérer et incrémenter l'ID
      const counterSnap = await getDoc(counterRef);
      let newId = counterSnap.exists() ? counterSnap.data().lastId + 1 : 1;
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.menuContainer}>
          {addresses.map((item) => (
  <View key={item.docId || item.id} style={styles.card}>
