...
const BadClients: React.FC = ({ searchQuery, setSearchQuery }) => {
  ...
  const renderBadClient = ({ item }) => (
    <View style={styles.badClientItem}>
      <Text style={styles.badClientName}>{item.clientName}</Text>
      <Text style={styles.badClientRemark}>{item.remark}</Text>
      <Text style={styles.reportedAt}>{new Date(item.reportedAt.seconds * 1000).toLocaleString()}</Text>
    </View>
  );
