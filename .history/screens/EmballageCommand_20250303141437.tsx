...
const EmballageCommand = ({ navigation }) => {
  ...
  const [successModalVisible, setSuccessModalVisible] = useState(false); // État pour le modal de succès
  ...
  const handleOrder = useCallback(async () => {
    ...
    try {
      ...
      setSuccessModalVisible(true); // Afficher le modal de succès
      setQuantity(""); // Réinitialiser la quantité après la commande
    } catch (error) {
        ...
        <View style={styles.modalBackground}>
          <View style={styles.modalContainer}>
            ...
            <Text style={styles.modalDetails}>
              {selectedSizeData.label} - {selectedSizeData.dimensions}
              {"\n"}
              Prix total: {totalPrice.toFixed(2)} Dinars
            </Text>
            <TouchableOpacity
