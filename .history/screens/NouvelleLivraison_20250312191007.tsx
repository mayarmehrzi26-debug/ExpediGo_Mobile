...
export const NouvelleLivraison: React.FC = () => {
  ...
  const [Status, setStatus] = useState<string | null>(null); // Pour stocker le statut par défaut
 ...
 useEffect(() => {
  const fetchStatus = async () => {
    const querySnapshot = await getDocs(collection(firebasestore, "Status"));
    const StatusOptions = querySnapshot.docs.map((doc) => ({
      label: doc.data().nomStat,
      value: doc.id,
    }));
...
const saveDelivery = async () => {
  ...
  try {
    ...
    const deliveryData = {
      ...
      termsAccepted,
      createdAt: new Date(),
      status: StatusOptions, // Utiliser le statut par défaut
    };
