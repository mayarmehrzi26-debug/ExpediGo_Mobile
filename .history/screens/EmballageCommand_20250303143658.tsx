...
interface Commande {
  id: string; // Ajout de l'ID pour chaque commande
  date: string; // Date de création
  quantite: number; // Quantité
  taille: string; // Taille
  pu: number; // Prix unitaire
  pt: number; // Prix total
  etat: string; // État de la commande
  dateAction: string; // Date d'action
}
      ...
      <ScrollView style={styles.scrollContent}>
        ...
        {commandes.map((commande) => (
          <View key={commande.id} style={styles.tableRow}>
            <Text style={styles.tableCell}>{commande.date}</Text>
            <Text style={styles.tableCell}>{commande.quantite}</Text>
            <Text style={styles.tableCell}>{commande.taille}</Text>
            <Text style={styles.tableCell}>{commande.pu} Dinars</Text>
            <Text style={styles.tableCell}>{commande.pt} Dinars</Text>
            <Text style={[styles.tableCell, styles.statusCell(commande.etat)]}>{commande.etat}</Text>
            <Text style={styles.tableCell}>{commande.dateAction}</Text>
          </View>
        ))}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  statusCell: (etat: string) => ({
    color: etat === "Livré" ? "green" : etat === "Refusé" ? "red" : "orange",
  }),
  bottomNav: {
    flexDirection: "row",
