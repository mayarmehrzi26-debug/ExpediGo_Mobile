...
interface CashBoxProps {
  // Aucune prop nécessaire, car les données sont récupérées directement dans le composant
}
...
const CashBox: React.FC<CashBoxProps> = () => {
  ...
  const [cashData, setCashData] = useState({
    availableCash: '0 DT',
    unavailableCash: '0 DT',
    totalCash: '0 DT',
  });

  // Fonction pour récupérer les données de la caisse depuis Firestore
  const fetchCashData = async () => {
    try {
      ...
      if (soldeDoc.exists()) {
        ...
        setCashData({
          availableCash: `${data.availableCash || 0} DT`,
          unavailableCash: `${data.unavailableCash || 0} DT`,
          totalCash: `${data.totalCash || 0} DT`,
        });
      } else {
        console.warn("Aucune donnée de caisse trouvée.");
      }
    } catch (error) {
  ...
  // Appeler fetchCashData au chargement du composant
  useEffect(() => {
    fetchCashData();
  }, []);
    ...
    <>
      <View style={styles.cashBox}>
        ...
        <View style={styles.cashRow}>
          ...
          <Text style={styles.cashValueBlue}>{cashData.availableCash}</Text>
        </View>
        <View style={styles.cashRow}>
          <Text style={styles.cashLabel}>Cash non Disponible</Text>
          <Text style={styles.cashValue}>{cashData.unavailableCash}</Text>
        </View>
        <View style={styles.divider} />
        <View style={styles.cashRow}>
          <Text style={[styles.cashLabel, styles.bold]}>Total</Text>
          <Text style={[styles.cashValue, styles.bold]}>{cashData.totalCash}</Text>
        </View>
