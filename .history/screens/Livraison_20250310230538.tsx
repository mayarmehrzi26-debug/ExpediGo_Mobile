...
import StatusBadge from "../src/components/StatusBadge"; // Assurez-vous que le chemin est correct
    ...
    const fetchDeliveries = async () => {
      try {
        ...
        const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
          const data = doc.data();
          console.log(`Delivery ID: ${doc.id}, Status ID: ${data.nomStat}`); // Log pour le statut
          ...
          const statusData = statusSnapshot.docs.find(statusDoc => statusDoc.id === data.nomStat)?.data();

          console.log(`Delivery ID: ${doc.id}, Status Data:`, statusData); // Log pour le statut
          ...
          return {
            ...
            date,
            status: statusData?.nomStat || "Statut inconnu",
          };
        }));
