...
import Header from "../src/components/Header"; // Assurez-vous que ce chemin est correct
    ...
    const fetchDeliveries = async () => {
      try {
        ...
        const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
          ...
          const productDoc = await getDoc(DocumentData(firebasestore, "products", data.product));
          const productData = productDoc.data();

          // Récupérer les détails du client
          const clientDoc = await getDoc(DocumentData(firebasestore, "clients", data.client));
          const clientData = clientDoc.data();

          // Récupérer les détails de l'adresse
          const addressDoc = await getDoc(DocumentData(firebasestore, "adresses", data.address));
          const addressData = addressDoc.data();

          return {
            id: doc.id,
            client: clientData?.name || "Client inconnu", // Afficher le nom du client
