...
import { v4 as uuidv4 } from 'uuid'; // Importer uuid pour générer des codes uniques
...
export const NouvelleLivraison: React.FC = () => {
  ...
  const saveDelivery = async () => {
    ...
    try {
      ...
      const trackingCode = uuidv4(); // Code de suivi unique

      // Générer le code QR
      const qrCodeUrl = generateQRCode(newId.toString());

      // Créer les données de livraison
      const deliveryData = {
        id: newId,
        trackingCode, // Ajouter le code de suivi ici
      ...
      alert(`Livraison enregistrée avec succès ! Code de suivi : ${trackingCode}`);
      navigation.goBack();
    } catch (error) {
