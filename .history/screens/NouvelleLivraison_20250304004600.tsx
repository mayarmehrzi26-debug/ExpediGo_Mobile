  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const productOptions = querySnapshot.docs.map(doc => ({
        label: doc.data().name,
        value: doc.id,  // Use the product ID or other unique identifier
      }));
    ...
    ...products.map(product => ({
      label: (
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          {/* Image pour chaque produit, en supposant que l'image est dans product.image */}
          <Image
            source={{ uri: product.image }} // Assurez-vous que chaque produit a une propriété 'image' contenant l'URL de l'image
