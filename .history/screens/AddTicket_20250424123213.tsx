...
const AddTicket: React.FC<{ route: any }> = ({ route }) => {
  const { refreshTickets } = route.params || {}; // Récupérer refreshTickets depuis params
  const dispatch = useDispatch<AppDispatch>(); // Utilisation de dispatch avec type
  const { tickets, loading, error } = useSelector((state: RootState) => state.tickets); // Sélectionner l'état Redux
  ...
  const handleAddTicket = async () => {
    ...
    try {
      ...
      dispatch(addTicket(ticketData)); // Ajoute le ticket à l'état Redux
