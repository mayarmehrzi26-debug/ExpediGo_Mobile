...
const Pickups: React.FC = () => {
  ...
  const exportSelectedDeliveries = async () => {
    ...
    if (selectedDeliveries.length === 0) {
      Alert.alert("Aucune livraison sélectionnée", "Veuillez sélectionner au moins une livraison à exporter.");
      return;
    }

    // Convertir les livraisons en format JSON
    const exportData = JSON.stringify(selectedDeliveries, null, 2);
    const filePath = `${RNFS.DocumentDirectoryPath}/livraisons_exportees.json`;

    try {
      await RNFS.writeFile(filePath, exportData, 'utf8');
      Alert.alert("Succès", `Livraisons exportées avec succès vers : ${filePath}`);
    } catch (error) {
      console.error("Erreur lors de l'exportation des livraisons :", error);
      Alert.alert("Erreur", "Une erreur est survenue lors de l'exportation des livraisons.");
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <FileExplorer 
        ...
        exportSelectedDeliveries={exportSelectedDeliveries} // Passer la fonction d'export
      />
