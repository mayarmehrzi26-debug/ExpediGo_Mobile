...
const Pickups: React.FC = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal visible={detailModalVisible} onDismiss={() => setDetailModalVisible(false)}>
        <View style={styles.modalContent}>
          <Text style={{ fontSize: 16, fontWeight: "bold" }}>Détails de la commande</Text>
          {selectedDeliveryDetails && (
            <>
              <Text>ID: {selectedDeliveryDetails.id}</Text>
              <Text>Client: {selectedDeliveryDetails.client}</Text>
              <Text>Adresse: {selectedDeliveryDetails.address}</Text>
              <Text>Status: {selectedDeliveryDetails.status}</Text>
              <Text>Paiement: {selectedDeliveryDetails.payment} DT</Text>
              <Text>Date: {selectedDeliveryDetails.date}</Text>
            </>
          )}
          <Button onPress={() => setDetailModalVisible(false)}>Fermer</Button>
        </View>
      </Modal>
    </View>
  );
