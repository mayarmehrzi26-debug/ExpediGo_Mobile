    ...
    const fetchPackageDetails = async () => {
      ...
      try {
        ...
        if (deliveryDoc.exists()) {
          ...
          // Récupération des détails de l'adresse de pickup
          const originDoc = await getDoc(doc(firebasestore, 'adresses', deliveryData.address));
          const originValue = originDoc.exists() ? originDoc.data()?.address : "Adresse inconnue";
          
          const mergedData: PackageDetails = {
            deliveryId: deliveryData.id,
            address: addressValue,
            origin:deliveryData.address,
            client: clientName,
    ...
    <View style={styles.container}>
      ...
      {packageDetails ? (
        <>
          ...
          <Text style={styles.dataText}>Destination: {packageDetails.address}</Text>
