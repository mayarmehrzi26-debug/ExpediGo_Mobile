...
const MesCommandes: React.FC = () => {
  ...
  const groupedDeliveries = deliveries.reduce((acc, delivery) => {
    const status = delivery.status || "Statut inconnu"; // Assurez-vous d'avoir un statut valide
  ...
  const filteredDeliveries = Object.keys(groupedDeliveries).reduce((acc, status) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {Object.keys(filteredDeliveries).map((status) => (
          <View key={status}>
            <Text style={styles.statusSectionTitle}>{status}</Text>
            {filteredDeliveries[status].map((delivery) => (
              <CommandeCard  />
            ))}
          </View>
        ))}
