...
const PackageDetailsLiv: React.FC<PackageDetailsProps> = ({
  ...
  const [locationSubscription, setLocationSubscription] =
    ...
    const startBackgroundTracking = async () => {
      try {
        ...
        // 2. S'assurer que l'app est au premier plan
        const appState = AppState.currentState;
        if (appState !== 'active') {
          Alert.alert(
            "L'application doit être active",
            "Veuillez garder l'application au premier plan pour démarrer le suivi",
            [{ text: "OK" }]
          );
          return false;
        }
    
        // 3. Démarrer le service en premier plan
        await Location.startLocationUpdatesAsync(LOCATION_TASK_NAME, {
          ...
          foregroundService: {
            ...
            notificationIcon: "ic_notification" // Assurez-vous d'avoir cette ressource
          },
        ...
        // Gestion spécifique pour l'erreur de foreground service
        if (error.message.includes('foreground service')) {
          Alert.alert(
            "Action requise",
            "Veuillez d'abord mettre l'application au premier plan, puis relancer le suivi",
            [{ text: "OK" }]
          );
        } else {
          Alert.alert(
            "Erreur",
            "Impossible de démarrer le suivi. Vérifiez les permissions de localisation.",
            [
  ...
  const stopBackgroundTracking = async () => {
    try {
      ...
      console.log(
        "Le suivi en arrière-plan était déjà arrêté ou n'existait pas"
      );
    }
  };
      ...
      try {
        ...
        if (deliveryDoc.exists()) {
          ...
          const clientDoc = await getDoc(
            doc(firebasestore, "clients", deliveryData.client)
          );
          ...
          const destinationDoc = await getDoc(
            doc(firebasestore, "clients", deliveryData.client)
          );
          const destinationName = destinationDoc.exists()
            ? destinationDoc.data()?.address
            : "Adresse inconnue";

          // Récupérer le nom du produit
          const productDoc = await getDoc(
            doc(firebasestore, "products", deliveryData.product)
          );
          const productValue = productDoc.exists()
            ? productDoc.data()?.name
            : "Produit inconnu";

          // Récupérer les détails de l'adresse
          const addressDoc = await getDoc(
            doc(firebasestore, "adresses", deliveryData.address)
          );
          const addressValue = addressDoc.exists()
            ? addressDoc.data()?.address
            : "Adresse inconnue";
          ...
          console.error(
            `Aucune livraison trouvée avec le code de suivi: ${scannedData}`
          );
        }
      } catch (error) {
        console.error(
  ...
  const confirmDelivery = async () => {
    ...
    try {
      ...
      setPackageDetails((prevDetails) => ({
        ...prevDetails,
        status:
          prevDetails?.status === "En cours de livraison"
            ? "Livré"
            ...
            <>
              ...
              <View style={styles.descriptionContainer}>
                <Text style={styles.descriptionText}>
                  Colis de{" "}
                  <Text style={styles.boldText}>{packageDetails.address}</Text>{" "}
                  à{" "}
