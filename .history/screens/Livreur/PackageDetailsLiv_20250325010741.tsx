...
const PackageDetailsLiv: React.FC<PackageDetailsProps> = ({ route, navigation }) => {
  ...
  TaskManager.defineTask(LOCATION_TASK_NAME, ({ data, error }) => {
    ...
    if (data) {
      ...
      if (location) {
        console.log('Location en arrière-plan:', location);
        // Ici vous pourriez envoyer la position à votre backend
      }
    }
  });
  ...
  try {
    const newStatus = packageDetails?.status === "En cours de livraison" 
      ? "Livré" 
    ...
    Alert.alert('Succès', newStatus === "Livré" 
...
const confirmDelivery = async () => {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Contenu existant dans une carte scrollable */}
...
const styles = StyleSheet.create({
  ...
  mapContainer: {
    height: 300, // Taille réduite de la carte
    width: '100%',
  },
  map: {
  ...
  contentScrollView: {
    flex: 1,
    marginTop: -30, // Permet de superposer légèrement la carte
