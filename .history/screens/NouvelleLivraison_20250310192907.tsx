...
useEffect(() => {
  const fetchDefaultStatus = async () => {
    try {
      ...
      const defaultStatusDoc = statusSnapshot.docs.find(doc => doc.data().nomStat === "en attente");
      if (defaultStatusDoc) {
        setDefaultStatus(defaultStatusDoc.id); // Récupérer l'ID du statut par défaut
        console.log("Statut par défaut récupéré :", defaultStatusDoc.data());
      } else {
        console.warn("Aucun statut par défaut trouvé.");
      }
    } catch (error) {
...
const saveDelivery = async () => {
  ...
  // Vérifiez que defaultStatus est défini avant d'enregistrer
  if (!defaultStatus) {
    alert("Le statut par défaut n'est pas disponible. Veuillez réessayer.");
    return;
  }
  ...
  try {
    ...
    const qrCodeUrl = generateQRCode(deliveryRef.id);
