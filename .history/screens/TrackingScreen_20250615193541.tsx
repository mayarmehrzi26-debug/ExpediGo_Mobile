...
const TrackingScreen = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // Géocodage des deux adresses avec délai pour respecter les limites de taux
        const from = await geocodeWithNominatim(fromAddress);
        await new Promise(resolve => setTimeout(resolve, 1000)); // Attendre 1 seconde entre les requêtes
        const to = await geocodeWithNominatim(toAddress);
