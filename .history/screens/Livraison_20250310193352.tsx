    ...
    const fetchDeliveries = async () => {
      try {
        ...
        const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
          ...
          const statusSnapshot = await getDocs(collection(firebasestore, "adresses"));
          const statusData = addressSnapshot.docs.find(statusdocs => statusdocs.id === data.nomStat)?.data();
          ...
          return {
            ...
            date,
            status: statusData?.nomStat || "En attente", // Ajouter un champ "status"
          };
        }));
