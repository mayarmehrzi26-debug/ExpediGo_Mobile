...
const TrackingScreen = () => {
  ...
  // Fonction pour extraire des coordonnées directement d'une adresse si possible
  const getSimpleCoordinates = (address: string): Coordinate | null => {
  ...
  // Géocodage avec LocationIQ avec gestion du cache et des erreurs
  const geocodeWithLocationIQ = async (address: string): Promise<Coordinate> => {
    ...
    // Essayer d'extraire les coordonnées directement
    const simpleCoords = getSimpleCoordinates(address);
    if (simpleCoords) {
      geocodeCache.set(address, simpleCoords);
      return simpleCoords;
    }

    try {
      // Ajouter un délai pour éviter les limites de taux
      await new Promise(resolve => setTimeout(resolve, 1100)); // 1.1s entre les requêtes
      ...
      throw new Error('Service de géocodage indisponible. Veuillez réessayer plus tard.');
    }
  };
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Géocodage des deux adresses avec gestion des erreurs individuelles
        let from: Coordinate | null = null;
        let to: Coordinate | null = null;
        
        try {
          from = await geocodeWithLocationIQ(fromAddress);
        } catch (err) {
          console.error('Erreur géocodage départ:', err);
          setError(`Impossible de localiser l'adresse de départ: ${err.message}`);
        }
        
        try {
          to = await geocodeWithLocationIQ(toAddress);
        } catch (err) {
          console.error('Erreur géocodage arrivée:', err);
          setError(prev => 
            prev ? `${prev}\nImpossible de localiser l'adresse d'arrivée: ${err.message}` 
                 : `Impossible de localiser l'adresse d'arrivée: ${err.message}`
          );
        }
        ...
        if (from && to) {
          ...
          // Si nous avons au moins une coordonnée, afficher ce que nous avons
          setFromCoords(from);
          setToCoords(to);
        }
  ...
  const handleOpenInBrowser = () => {
    if (fromCoords && toCoords) {
      ...
      Linking.openURL(
        `https://www.openstreetmap.org/search?query=${encodeURIComponent(fromAddress)}%20to%20${encodeURIComponent(toAddress)}`
      );
    }
  };
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.errorText}>Problème de localisation</Text>
          ...
          <View style={styles.addressBox}>
            ...
            {fromCoords && (
              <Text style={styles.coordsText}>
                Localisé: {fromCoords.latitude.toFixed(4)}, {fromCoords.longitude.toFixed(4)}
              </Text>
            )}
          </View>
          ...
          <View style={styles.addressBox}>
            ...
            {toCoords && (
              <Text style={styles.coordsText}>
                Localisé: {toCoords.latitude.toFixed(4)}, {toCoords.longitude.toFixed(4)}
              </Text>
            )}
          </View>
