...
const Support: React.FC = () => {
  ...
  const addTicket = (newTicket) => {
    setTickets([...tickets, newTicket]); // Ajouter un nouveau ticket à la liste
  };
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.container}>
          {tickets.map((ticket, index) => (
            <InvoiceDocument key={index} {...ticket} />
          ))}
        </View>
      </ScrollView>
      <TouchableOpacity
        ...
        onPress={() => {
          toggleButtonMode();
          navigation.navigate("AddTicket", { addTicket }); // Passer la fonction addTicket à l'écran AddTicket
        }}
      >
