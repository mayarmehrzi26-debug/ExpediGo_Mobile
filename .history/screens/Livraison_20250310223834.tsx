    ...
    const fetchDeliveries = async () => {
      try {
        ...
        const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
          ...
          const statusData = statusSnapshot.docs.find(statusDoc => statusDoc.id === data.status)?.data(); // Assurez-vous d'utiliser `data.status`
          ...
          return {
            ...
            date,
            status: statusData?.nomStat || "Statut inconnu", // Utilisez `nomStat` comme clé
          };
        }));
