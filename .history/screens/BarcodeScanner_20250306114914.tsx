...
export default function BarcodeScanner({ onClose }: BarcodeScannerProps) {
  ...
  const handleBarcodeScanned = ({ data }: { data: string }) => {
    setScannedData(data); // Stocker les données scannées
    // Vous pouvez également fermer la caméra automatiquement après un scan réussi si nécessaire
    // onClose();
  };
// Fonction appelée lorsqu'un code à barre est scanné
const handleBarcodeScanned = async ({ data }: { data: string }) => {
  ...
  if (packageData) {
    ...
    // Vous pouvez mettre à jour l'état pour afficher les détails du package dans l'interface utilisateur
  } else {
...
{scannedData && (
  <View style={styles.scannedDataContainer}>
    ...
    <Text style={styles.scannedDataText}>Adresse: {livraison.adresse}</Text>
    <Text style={styles.scannedDataText}>Produit: {livraison.produit}</Text>
    <Text style={styles.scannedDataText}>Date: {livraison.date}</Text>
