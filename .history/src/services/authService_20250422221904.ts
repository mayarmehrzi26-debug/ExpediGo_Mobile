...
// Extrait de authService.ts
const sendWelcomeEmail = async (user: User, tempPassword: string) => {
  ...
  try {
    ...
    // (Intégration avec SendGrid/Mailjet etc.)
    await sendCustomWelcomeEmail(user.email, tempPassword);
