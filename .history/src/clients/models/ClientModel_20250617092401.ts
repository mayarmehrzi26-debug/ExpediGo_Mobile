...
export class ClientService {
 private static async createClientUserAccount(
  email: string,
  password: string,
  clientId: string,
  originalUser: User
  ...
  try {
    // Sauvegarder le token de l'utilisateur actuel
    const originalIdToken = await firebaseAuth.currentUser?.getIdToken();
    ...
    await setDoc(doc(firebasestore, "users", newUser.uid), {
      uid: newUser.uid,
      email,
      role: "destinataire",
      clientId
    ...
    // Restaurer la session originale en se reconnectant
    if (originalIdToken) {
      await firebaseAuth.signInWithCustomToken(originalIdToken);
    }
  } catch (error) {
...
static async addClientWithEmail(
  ...
  try {
    ...
    // Créer le compte utilisateur (version simplifiée)
    await this.createClientUserAccount(client.email, password, docRef.id);
