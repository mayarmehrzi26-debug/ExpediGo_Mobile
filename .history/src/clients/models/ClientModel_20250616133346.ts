...
static async createUserAccount(
  ...
  try {
    ...
    // Reconnexion de l'utilisateur admin original s'il existait
    if (currentUser) {
      await firebaseAuth.signOut();
      await firebaseAuth.signInWithEmailAndPassword(
        currentUser.email!, 
        "mot_de_passe_admin" // Vous devrez gérer cela sécuritairement
      );
    } else {
      await firebaseAuth.signOut();
    }
    ...
    throw error; // Laissez le presenter gérer l'erreur
  }
}
