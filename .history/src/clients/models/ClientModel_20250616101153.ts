...
static async addClientWithEmail(
  ...
  try {
    const backendUrls = [
    'http://192.168.117.160',  // URL primaire
    'http://192.168.117.161',  // URL secondaire
    'http://backup-server:8080'  // URL de backup
  ];

    
    // 1. D'abord essayer d'envoyer l'email
    const emailResponse = await fetch(`${backendUrls}/send-welcome-email`, {
      method: 'POST',
      headers: {
