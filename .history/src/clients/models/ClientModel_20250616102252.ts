...
static async addClientWithEmail(
  ...
  const backendUrls = [
    'http://192.168.117.160:3000',  // IP locale + port explicite
    'http://localhost:3000',        // Fallback local
    'http://192.168.117.161:3000'   // Backup
  ];

  let lastError: Error | null = null;
  let emailSent = false;

  // Essayer chaque URL jusqu'à réussite
  for (const url of backendUrls) {
    try {
      ...
      if (!emailResult.success) {
        throw new Error(emailResult.error || "Échec côté serveur");
      }

      emailSent = true;
      break; // Sortir de la boucle si réussi
    } catch (error) {
      console.error(`Échec avec ${url} :`, error.message);
      lastError = error;
    }
  }
  ...
  // Créer le client seulement si l'email est envoyé
  try {
    const docRef = await addDoc(collection(firebasestore, "clients"), client);
    return {
      id: docRef.id,
    ...
    throw new Error("Client créé mais erreur de suivi");
  }
}
