...
static async addClientWithEmail(
  ...
  const backendUrls = [
    'http://20.20.0.63:3001',  // IP principale
    'http://localhost:3001',   // Fallback local
    'http://192.168.117.160:3001' // Fallback réseau local
  ];
  ...
  for (const url of backendUrls) {
    try {
      ...
      // Si l'email est envoyé, créer le client
      const docRef = await addDoc(collection(firebasestore, "clients"), client);
      ...
      console.error(`Échec avec ${url}:`, error);
      lastError = error;
    }
  }
