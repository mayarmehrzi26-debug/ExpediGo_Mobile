import { Auth, createUserWithEmailAndPassword, signInWithEmailAndPassword } from "firebase/auth";import { addDoc, collection, deleteDoc, doc, getDoc, getDocs, query, setDoc, updateDoc } from "firebase/firestore";
...
static async createUserAccount(
  ...
  try {
    // 1. Sauvegarder l'utilisateur actuel
    ...
    // 3. Reconnecter l'utilisateur original si nécessaire
    if (currentUser && currentUserToken) {
      await firebaseAuth.signOut(); // Se déconnecter du nouveau compte
      await firebaseAuth.signInWithCustomToken(currentUserToken); // Reconnecter l'original
    }
