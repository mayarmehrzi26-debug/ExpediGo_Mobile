...
export class ClientService {
  static async addClient(client: Omit<Client, "id">, senderId: string): Promise<string> {
    try {
      const clientWithSender = {
        ...client,
        senderId,
        createdAt: new Date().toISOString()
      };
      const docRef = await addDoc(collection(firebasestore, "clients"), clientWithSender);
      return docRef.id;
    } catch (error) {
  ...
  static async addClientWithEmail(
    ...
    try {
      ...
      // 2. Créer le client avec l'expéditeur
  ...
  static async getClients(senderId: string): Promise<Client[]> {
    try {
      const q = query(
