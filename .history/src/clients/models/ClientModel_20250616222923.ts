...
static async addClientWithEmail(
    ...
    for (const url of backendUrls) {
      try {
        ...
        // Utilisez la nouvelle méthode pour créer le compte
        await this.createUserWithoutSignOut(client.email, password, docRef.id);
...
static async createUserAccount(
  ...
  try {
    ...
    if (currentUser) {
      // Vous devrez peut-être stocker le token de l'utilisateur original
      // et le réutiliser ici pour le reconnecter
    }
