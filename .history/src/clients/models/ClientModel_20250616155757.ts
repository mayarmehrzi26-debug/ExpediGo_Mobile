...
static async createUserAccount(
  ...
  try {
    // 1. Sauvegarder l'utilisateur actuel avant création
    ...
    // 3. Enregistrer les infos supplémentaires
    const userData = {
      uid: user.uid,
      email,
      role: "client",
    ...
    // 4. Déconnecter immédiatement le nouveau user
    await signOut(firebaseAuth);

    // 5. Reconnecter l'utilisateur original si existant
    if (currentUser) {
      await signInWithEmailAndPassword(
        firebaseAuth,
        currentUser.email!,
        "password_admin" // Vous devrez gérer ceci sécuritairement
      );
    }
