...
static async createUserAccount(
  ...
  try {
    // Sauvegarder l'email de l'admin connecté
    ...
    if (currentUserEmail) {
      ...
      await signInWithEmailAndPassword(firebaseAuth, currentUserEmail, "password_admin"); // Vous devrez gérer le mot de passe
    } else {
      await firebaseAuth.signOut();
    }
