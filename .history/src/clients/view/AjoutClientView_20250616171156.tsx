 ...
 const presenter = new AjoutClientPresenter({
    ...
    showSuccess: (title, message, callback) => {
      Alert.alert(
        title, 
        message,
        callback ? [{ text: "OK", onPress: callback }] : undefined,
        { 
          onDismiss: () => {
            if (callback) {
              setTimeout(callback, 500); // Petite temporisation avant le callback
            }
          }
        }
      );
