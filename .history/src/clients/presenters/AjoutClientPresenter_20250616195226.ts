...
export interface AjoutClientView {
  ...
  showAutoDismissSuccess(title: string, message: string, duration?: number, callback?: () => void): void;
  navigateBack(): void;
  resetForm(): void; 
}
...
async handleSubmit(clientData: {
  ...
  try {
    ...
    this.view.showAutoDismissSuccess(
      ...
      5000,
      () => {
        this.view.resetForm(); // Réinitialisation du formulaire
        this.view.navigateBack(); // Navigation en arrière
      }
    );
