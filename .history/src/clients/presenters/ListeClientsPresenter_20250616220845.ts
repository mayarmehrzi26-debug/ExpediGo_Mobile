...
export class ListeClientsPresenter {
 ...
 async loadClients() {
  try {
    ...
    const clients = await ClientService.getClientsByCurrentUser();
    console.log("Clients loaded:", clients); // Ajouté
    this.view.setClients(clients);
  } catch (error: any) {
    console.error("Error loading clients:", error); // Ajouté
    this.view.showError("Erreur", error.message || "Impossible de charger les clients");
    this.view.setClients([]); // S'assurer que la liste est vide en cas d'erreur
  } finally {
