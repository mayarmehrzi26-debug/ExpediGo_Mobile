...
export class AjoutClientPresenter {
  ...
  async handleSubmit(clientData: {
    ...
    try {
      ...
      const user = await ClientService.createUserAccount(
        clientData.email, 
        randomPassword, 
        "" // clientId vide pour l'instant
      );

      // Ensuite ajouter le client avec l'ID de l'utilisateur créé
      ...
      // Mettre à jour l'utilisateur avec le clientId
      await ClientService.updateUserClientId(user.uid, clientId);
