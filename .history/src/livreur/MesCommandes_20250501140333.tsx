  ...
  const loadData = async () => {
    ...
    try {
      const [mesCommandes, mesEmballages] = await Promise.all([
        fetchMesLivraisons(),
        fetchUserEmballages() // Utilisez la nouvelle fonction
      ]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabsContainer}>
        ...
        >
          <Text style={[styles.tabText, activeTab === 'emballages' && styles.activeTabText]}>Mes Emballages</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollView}>
        {loading ? (
          ...
          filteredEmballages.length === 0 ? (
            <Text style={styles.emptyText}>
              {emballages.length === 0 
                ? "Vous n'avez pas encore passé de commande d'emballage" 
