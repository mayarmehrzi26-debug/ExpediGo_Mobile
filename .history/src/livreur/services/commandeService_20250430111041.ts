...
(NOBRIDGE) ERROR  Erreur lors du chargement des commandes: [ReferenceError: Property 'getAuth' doesn't exist]

interface Livraison {
  id?: string;
  client: string;
  address: string;
  status: "Non traité" | "En attente d'enlèvement" | "Traité";
  createdAt: any;
...
export const updateLivraisonStatus = async (livraisonId: string, status: "Non traité" | "En attente d'enlèvement" | "Traité") => {
  ...
  try {
    await updateDoc(livraisonRef, { 
      ...
      // par exemple, la date de mise à jour
