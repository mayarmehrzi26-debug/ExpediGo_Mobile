...
export const fetchLivraisons = async () => {
  ...
  for (const docSnap of querySnapshot.docs) {
    ...
    const [clientSnap, addressSnap, userSnap, createdBySnap] = await Promise.all([
      getDoc(doc(firebasestore, "clients", data.client)),
      getDoc(doc(firebasestore, "adresses", data.address)),
      getDoc(doc(firebasestore, "users", data.users)),
      data.createdBy ? getDoc(doc(firebasestore, "users", data.createdBy)) : Promise.resolve(null)
    ]);
    ...
    livraisons.push({
      ...
      expediteurName: usersData?.name || "",
      createdByName: createdByData?.name || "",
      createdByPhone: createdByData?.phone || "",
      type: "livraison"
    });
  }
...
export const fetchMesLivraisons = async (): Promise<any[]> => {
  ...
  try {
    ...
    const livraisons = await Promise.all(querySnapshot.docs.map(async (docSnap) => {
      ...
      // Récupération des données client, adresse et createdBy
