...
export const getCommandHistory = async (commandeId: string) => {
  try {
    const historyRef = collection(firebasestore, "livraisons", commandeId, "history");
    ...
    return querySnapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
      // Convertir le timestamp Firestore en date JS si nécessaire
...
export const updateLivraisonStatus = async (
 ...
 await addHistoryEntry(
      ...
      user.uid,
      'livreur', // ou déterminer le rôle dynamiquement
      { newStatus: status }
    );
