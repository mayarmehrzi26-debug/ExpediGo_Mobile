...
interface Livraison {
  ...
  client: string;
  address: string;
  status: "Non traité" | "En attente d'enlèvement" | "En cours" | "Traité";
  createdAt: any;
  assignedTo?: string;
}
...
export const fetchLivraisons = async () => {
  ...
  for (const docSnap of querySnapshot.docs) {
    ...
    livraisons.push({
      id: docSnap.id,
      origine: clientData?.address || "origine inconnue",
...
export const fetchMesLivraisons = async () => {
  ...
  console.log("Fetching deliveries for user:", user.uid);
  ...
  try {
    const querySnapshot = await getDocs(q);
    console.log(`Found ${querySnapshot.size} deliveries`);
