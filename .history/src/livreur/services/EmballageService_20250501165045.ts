...
const processEmballages = async (querySnapshot) => {
  ...
  for (const docSnap of querySnapshot.docs) {
    ...
    let latitude = 36.8085; // Valeur par défaut
    let longitude = 10.1835; // Valeur par défaut
    ...
    try {
      if (data.addressId) {
        ...
        if (addressDoc.exists()) {
          ...
          latitude = addressData?.latitude || latitude;
          longitude = addressData?.longitude || longitude;
        }
      }
    } catch (error) {
