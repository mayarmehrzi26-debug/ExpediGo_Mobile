...
const CardCommande: React.FC<CardProps> = ({ commande, onRefresh }) => {
  ...
  const getButtonText = () => {
    switch (commande.status) {
      case "Non traité": return "Commencer le pickup";
  ...
  const shouldShowButton = () => {
    return ["Non traité", "En cours de pickup", "Picked", "En cours de livraison"].includes(commande.status);
  };
      ...
      {shouldShowButton() && (
        <TouchableOpacity 
          style={[
            ...
            commande.status === "En cours de pickup" && styles.secondaryButton,
