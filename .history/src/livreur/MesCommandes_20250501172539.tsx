...
const MesCommandes: React.FC = () => {
  ...
  const filteredCommandes = commandes
    .filter((commande) => {
      switch (activeFilter) {
        case "À livrer":
          return commande.status === "En attente d'enlèvement";
        case "À récupérer":
          return commande.status === "Picked";
        case "En cours":
          return commande.status === "En cours de livraison";
        case "Livrée":
          return commande.status === "Livré";
        default:
          return true;
      }
    })
  ...
  const filteredEmballages = emballages
    .filter((emballage) => {
      switch (activeFilter) {
        case "En attente":
          return emballage.status === "En attente";
        case "Accepté":
          return emballage.status === "Accepté";
        case "Refusé":
          return emballage.status === "Refusé";
        default:
          return true;
      }
    })
    ...
    <View style={styles.container}>
      ...
      <View style={styles.tabsContainer}>
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            setActiveFilter("Toutes les commandes");
          }}
        >
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            setActiveTab('emballages');
            setActiveFilter("Tous les emballages");
          }}
        >
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterContainer}>
        {(activeTab === 'livraisons'
          ? ["Toutes les commandes", "À livrer", "À récupérer", "En cours", "Livrée"]
          : ["Tous les emballages", "En attente", "Accepté", "Refusé"]
        ).map((filter) => (
          <TouchableOpacity
            key={filter}
...
const styles = StyleSheet.create({
  ...
  filterButton: {
    width: 120, // ✅ largeur fixe pour tous les boutons
    height: 40, // ✅ hauteur fixe pour uniformité
