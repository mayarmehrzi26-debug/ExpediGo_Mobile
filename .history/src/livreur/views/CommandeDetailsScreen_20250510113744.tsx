...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  const handleStartConversation = async (userId, userName) => {
    try {
      ...
      if (!userExists) {
        Alert.alert("Erreur", "Le destinataire n'existe pas dans notre système");
        return;
      }
    ...
    <TouchableOpacity 
      ...
      onPress={() => {
        if (commande.client.id) {
          handleStartConversation(commande.client.id, commande.client.name);
        } else {
          Alert.alert("Erreur", "Destinataire invalide");
        }
      }}>
      <MaterialIcons name="message" size={24} color="#44076a" />
    </TouchableOpacity>
