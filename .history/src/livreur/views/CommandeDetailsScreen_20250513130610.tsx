 ...
 const handleStatusUpdate = async (newStatus) => {
  ...
  try {
    ...
    console.log('Détails avant mise à jour:', {
      ...
      newStatus,
      montant,
      expediteurId: commandeActuelle.createdBy?.id
    });

    if (newStatus === "Livré" && !commandeActuelle.createdBy?.id) {
    ...
    if (newStatus === "Livré" && commandeActuelle.createdBy?.id) {
      if (montant > 0) {
        console.log(`Tentative de crédit de ${montant}€ à ${commandeActuelle.createdBy.id}`);
