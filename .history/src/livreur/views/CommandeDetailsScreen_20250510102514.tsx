    ...
    try {
      ...
      Alert.alert(
        ...
        [
          ...
          { text: "Ouvrir le clavier", onPress: () => Linking.openURL(`telprompt:${cleanedPhoneNumber}`) }
        ]
      );
    }
  };
 ...
 const handleSendMessage = async () => {
  if (newMessage.trim() && currentChatUser && commande.id) {
    ...
    try {
      ...
      const participants = [user.uid, currentChatUser.id].sort();
      ...
      // Envoyer le message
      await livraisonModel.sendMessage(conversationId, {
        text: newMessage,
        sender: user.uid
      });
...
const loadMessages = async (conversationId: string) => {
  try {
    const messages = await livraisonModel.getMessages(conversationId);
    setMessages(messages.reverse()); // Inverser pour avoir les plus récents en bas
  } catch (error) {
...
const handleOpenChat = async (user) => {
  ...
  try {
    ...
    if (!conversationId) {
      // Si nouvelle conversation, ne pas charger de messages
      setMessages([]);
    } else {
      ...
      unsubscribeRef.current = livraisonModel.setupMessagesListener(
        conversationId, 
        (updatedMessages) => {
    ...
    <View style={styles.container}>
      ...
      >
                ...
                <>
                  <TouchableOpacity 
                    ...
                    onPress={() => handleOpenChat({
                      id: commande.createdBy.id,
                      name: commande.createdBy.name
                    })}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.contactCard}>
            <View style={styles.contactInfo}>
              <Text style={styles.contactName}>{commande.client.name || 'Non spécifié'}</Text>
