...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  const [livraisonModel] = useState(new LivraisonModel());
      ...
      if (distance < DISTANCE_THRESHOLD) {
        Alert.alert(
          ...
          [
            ...
            { text: "Problème", onPress: () => handleStatusUpdate("Retour"), style: "destructive" }
          ]
        );
      }
    }
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.contactCard}>
  <View style={styles.contactInfo}>
    <Text style={styles.contactName}>{commande.destinataire?.name || commande.client?.name || 'Non spécifié'}</Text>
    {commande.destinataire?.phone || commande.client?.phone ? (
      <Text style={styles.contactPhone}>{commande.destinataire?.phone || commande.client?.phone}</Text>
    ) : null}
  </View>
    ...
    {(commande.destinataire?.phone || commande.client?.phone) && (
      <>
        <TouchableOpacity 
          style={styles.iconButton}
          onPress={() => {
            const destinataireId = commande.destinataire?.id || commande.client?.id;
            const destinataireName = commande.destinataire?.name || commande.client?.name;
            
            if (destinataireId) {
              handleStartConversation(destinataireId, destinataireName);
            } else {
              Alert.alert("Info", "Destinataire non disponible pour le chat");
            }
          }}>
          <MaterialIcons name="message" size={24} color="#44076a" />
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          style={styles.iconButton}
          onPress={() => {
            const phoneNumber = commande.destinataire?.phone || commande.client?.phone;
            const contactName = commande.destinataire?.name || commande.client?.name;
            
            if (phoneNumber) {
              confirmCall(phoneNumber, contactName || 'Destinataire');
            }
          }}>
          <MaterialIcons name="phone" size={24} color="#44076a" />
        </TouchableOpacity>
