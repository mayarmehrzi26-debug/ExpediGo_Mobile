...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  const [commande, setCommande] = useState({
    ...initialCommande,
    paiement: initialCommande.paiement || 'non_payé', // Valeur par défaut
    montant: initialCommande.montant || 0
  ...
  const livraisonModel = useRef(new LivraisonModel()).current;  const [destinataire, setDestinataire] = useState(null);
  useEffect(() => {
      ...
      if (distance < DISTANCE_THRESHOLD) {
        Alert.alert(
          ...
          [
            ...
            { text: "Oui", onPress: () => handleStatusUpdate("Picked") }
          ]
        );
      }
    }
      ...
      if (distance < DISTANCE_THRESHOLD) {
        Alert.alert(
          ...
          [
            ...
            { text: "Retour", onPress: () => handleStatusUpdate("Retour"), style: "destructive" }
          ]
        );
      }
    }
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      if (success) {
        ...
        // Cas d'un retour
        if (newStatus === "Retour") {
          if (destinataire?.id || commande.client?.id) {
            await livraisonModel.incrementClientReturns(destinataire?.id || commande.client?.id);
          }
        }
        
        // Cas d'une livraison réussie avec paiement à percevoir
        if (newStatus === "Livré" && commande.paiement === "percevoir") {
          if (commande.createdBy?.id && commande.montant) {
            try {
              await livraisonModel.creditExpediteur(
                commande.createdBy.id, 
                commande.montant
              );
              
              // Mettre à jour le statut du paiement dans la commande
              await livraisonModel.updatePaymentStatus(commande.id, "payé");
              
              Alert.alert(
                'Succès', 
                `Livraison confirmée et ${commande.montant}€ crédités à l'expéditeur`
              );
            } catch (error) {
              console.error("Erreur lors du crédit:", error);
              Alert.alert(
                'Erreur', 
                'Livraison confirmée mais échec du crédit: ' + error.message
              );
            }
            return;
          } else {
            Alert.alert(
              'Avertissement', 
              'Livraison confirmée mais impossible de créditer l\'expéditeur (informations manquantes)'
            );
          }
        }
      ...
      case "En cours de livraison":
        Alert.alert(
          ...
          [
            ...
            { text: "Retour/Problème", onPress: () => handleStatusUpdate("Retour"), style: "destructive" }
          ]
        );
        break;
    }
  ...
  const getActionButtonInfo = () => {
    switch (commande.status) {
      case "Non traité": return { text: "Prendre en charge", color: "#44076a" };
      case "En attente d'enlèvement": return { text: "Commencer le pickup", color: "#877DAB" };
      case "Picked": return { text: "Commencer la livraison", color: "#5D8BF4" };
      case "En cours de livraison": return { text: "Terminer la livraison", color: "#ac0d8c" };
      default: return { text: "", color: "#44076a" };
    }
  };
