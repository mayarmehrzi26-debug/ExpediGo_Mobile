  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      if (success) {
        ...
        // Gestion des livraisons réussies avec paiement à percevoir
        if (newStatus === "Livré" && commande.paiement === "percevoir") {
          if (!commande.createdBy?.id) {
            throw new Error("ID expéditeur manquant");
          }
          
          if (!commande.montant || commande.montant <= 0) {
          ...
          await livraisonModel.updateLivraisonField(commande.id, { 
            paiement: "payé",
          ...
          Alert.alert(
            'Succès', 
            `Livraison confirmée et ${commande.montant}€ crédités à ${commande.createdBy.name}`
          );
          return;
        }
