...
const CommandeDetailsScreen = ({ route, navigation }) => {
    ...
    try {
      ...
      const sub = await Location.watchPositionAsync(
        ...
        async (newLocation) => {
          ...
          if (newLocation.coords.latitude === 0 && newLocation.coords.longitude === 0) {
            return;
          }
  ...
  useEffect(() => {
    // Initialize position tracking when component mounts
    const initializeTracking = async () => {
      try {
        ...
        await livraisonModel.initializeLivreurPosition(user.uid);
        
        // Start listening to position updates
        trackingUnsubscribeRef.current = livraisonModel.listenToLivreurPosition(
          user.uid,
          (position) => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      // Gestion du tracking selon le nouveau statut
      if (newStatus === "En cours de pickup") {
        await startTracking("En cours de pickup");
      } else if (newStatus === "En cours de livraison") {
        await startTracking("En cours de livraison");
