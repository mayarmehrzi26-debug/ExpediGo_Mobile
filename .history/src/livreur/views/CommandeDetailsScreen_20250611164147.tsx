...
const loadProducts = async () => {
  if (!commande?.products) return [];
  
  try {
    // Si les produits sont déjà complets dans la commande (avec name, description, etc.)
    if (commande.products[0]?.name) {
      return commande.products;
    }
    
    // Sinon, on charge les détails depuis Firestore
    const productsWithDetails = await Promise.all(
      commande.products.map(async (item) => {
    ...
    return commande.products || []; // Retourne les produits de base si erreur
  }
};

  // Appelez loadProducts dans un useEffect
useEffect(() => {
  const loadData = async () => {
    const loadedProducts = await loadProducts();
    setProducts(loadedProducts);
    
    // Si le montant total n'est pas défini, on le calcule
    if (!commande.totalAmount && loadedProducts.length > 0) {
      const calculatedAmount = loadedProducts.reduce((total, product) => {
  ...
  // Fonction pour mettre à jour la position du livreur dans Firestore
    ...
    try {
      ...
      await livraisonModel.updateOrCreateLivreurPosition(
        firebaseAuth.currentUser?.uid, 
        {
          ...
          assignedTo: firebaseAuth.currentUser?.uid // Ajout de l'ID du livreur
        }
      );
...
const handleStatusUpdate = async (newStatus) => {
  ...
  try {
    ...
    // Notification spécifique pour le livreur
    ...
    // Démarrer le tracking seulement pour les statuts spécifiques
    if (newStatus === "En cours de pickup" || newStatus === "En cours de livraison") {
      ...
      // Nettoyer la position du livreur
      await livraisonModel.updateOrCreateLivreurPosition(
        firebaseAuth.currentUser?.uid,
        {
          latitude: 0,
          ...
          {selectedProduct && (
            <>
              ...
              <View style={styles.productDetailRow}>
                ...
                <Text style={styles.productDetailValue}>{selectedProduct.price} DT</Text>
              </View>
...
<View style={styles.detailRow}>
            ...
            <Text style={[styles.detailValue, { fontWeight: 'bold' }]}>
              {commande.totalAmount.toFixed(2)} DT
            </Text>
          </View>
              ...
              >
                <View style={styles.productInfo}>
                  ...
                  <Text style={styles.productQuantity}>
                    {product.quantity} x {product.price} DT
                  </Text>
                </View>
