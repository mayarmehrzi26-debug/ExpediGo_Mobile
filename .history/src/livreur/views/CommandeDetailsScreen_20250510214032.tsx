...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  // Composant Bouton d'alerte personnalisé
  const AlertButton = ({ text, onPress, style }) => {
    let buttonStyle = styles.alertButton;
    let textStyle = styles.alertButtonText;
    
    if (style === 'cancel') {
      buttonStyle = [buttonStyle, styles.alertButtonCancel];
      textStyle = [textStyle, styles.alertButtonCancelText];
    } else if (style === 'destructive') {
      buttonStyle = [buttonStyle, styles.alertButtonDestructive];
      textStyle = [textStyle, styles.alertButtonDestructiveText];
    }
  ...
  const handleStartConversation = async (userId, userName) => {
    try {
      ...
      if (!recipientUser) {
        showCustomAlert(
          "Erreur",
          "Le destinataire n'existe pas dans le système",
          [{ text: "OK" }],
          "error"
        );
        return;
      ...
      >
        <View style={styles.alertOverlay}>
          <View style={[
            ...
            alertConfig.type === 'success' && styles.alertContainerSuccess,
            alertConfig.type === 'warning' && styles.alertContainerWarning,
            alertConfig.type === 'error' && styles.alertContainerError
          ]}>
            <MaterialIcons 
              name={
                alertConfig.type === 'success' ? 'check-circle' :
                alertConfig.type === 'warning' ? 'warning' :
                alertConfig.type === 'error' ? 'error' : 'info'
              } 
              size={32} 
              color={
                alertConfig.type === 'success' ? '#34A853' :
                alertConfig.type === 'warning' ? '#FBBC05' :
                alertConfig.type === 'error' ? '#EA4335' : '#4285F4'
              } 
