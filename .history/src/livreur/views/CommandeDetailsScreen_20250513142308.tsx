...
const checkProximity = (location) => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      console.log('Détails complets de la commande:', commandeActuelle);
    
      if (!commandeActuelle) {
        throw new Error("Commande introuvable");
      }
    
      // Vérification de l'expéditeur (createdBy)
      const expediteurId = commandeActuelle.createdBy?.id;
      
      if (!expediteurId) {
        throw new Error("Aucun expéditeur (createdBy) trouvé pour cette commande");
      }
  
      console.log(`Expéditeur à créditer: ${expediteurId}, Montant: ${montant}`);
