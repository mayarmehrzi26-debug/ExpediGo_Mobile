...
const checkProximity = (location) => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    setUpdating(true);
    try {
      console.log('[DEBUG] Début de handleStatusUpdate', {
        commandeId: commande.id,
        newStatus,
        payment: commande.payment,
        totalAmount: commande.totalAmount,
        createdById: commande.createdBy?.id
      });
  
      await updateLivraisonStatus(commande.id, newStatus);
      setCommande(prev => ({ ...prev, status: newStatus }));
  
      if (newStatus === "Livré" && (commande.payment === "percevoir" || commande.payment === "non_payé")){
        if (!commande.createdBy?.id || !commande.totalAmount) {
          const errorMsg = "Données manquantes pour le crédit: " + 
            `createdBy.id=${commande.createdBy?.id}, totalAmount=${commande.totalAmount}`;
          console.error('[ERROR] ' + errorMsg);
          throw new Error(errorMsg);
        }
  
        console.log('[DEBUG] Appel à creditExpediteur', {
          expediteurId: commande.createdBy.id,
        ...
        Alert.alert(
          'Succès', 
          `Livraison confirmée et ${commande.totalAmount}€ ajoutés au solde en attente de l'expéditeur`
        );
        return;
      }
  
      console.log('[DEBUG] Statut mis à jour sans crédit de solde');
      Alert.alert('Succès', 'Statut mis à jour');
    } catch (error) {
      console.error("[ERROR] Erreur dans handleStatusUpdate:", {
        error: error.message,
        stack: error.stack,
        commandeId: commande.id,
        status: newStatus
