...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  const [driverLocationSubscription, setDriverLocationSubscription] = useState(null);
  useEffect(() => {
...
// Ajoutez cette fonction pour suivre la position du livreur
const trackDriverLocation = async (driverId) => {
  if (!driverId) return;

  try {
    // Arrêtez tout abonnement existant
    if (driverLocationSubscription) {
      driverLocationSubscription();
    }

    const driverLocationRef = doc(firebasestore, "driverLocations", driverId);
    ...
    setDriverLocationSubscription(() => unsubscribe);
  } catch (error) {
...
useEffect(() => {
  if (commande?.assignedTo) {
    trackDriverLocation(commande.assignedTo);
  }
...
useEffect(() => {
  ...
  const updateLocation = async () => {
    if (commande?.assignedTo === firebaseAuth.currentUser?.uid) {
      try {
        const location = await Location.getCurrentPositionAsync({});
        await livraisonModel.updateDriverLocation(
        ...
        console.warn('Erreur de mise à jour de la position:', error);
      }
    }
  };
  ...
  if (commande?.assignedTo === firebaseAuth.currentUser?.uid) {
    updateLocation(); // Mise à jour immédiate
    interval = setInterval(updateLocation, 10000); // Mise à jour toutes les 10s
  }
