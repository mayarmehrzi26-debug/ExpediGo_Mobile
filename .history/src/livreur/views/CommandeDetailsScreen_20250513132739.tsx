...
const checkProximity = (location) => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      // DÉTERMINER QUI DOIT ÊTRE CRÉDITÉ
      const destinataireACrediter = destinataire?.uid || commandeActuelle.clientId;
      const expediteurACrediter = commandeActuelle.createdBy?.id;
  
      // Logique pour décider qui créditer (à adapter selon vos règles métier)
      const beneficiareDuCredit = 
        commande.payment === "payé_par_expediteur" ? destinataireACrediter :
        commande.payment === "payé_par_destinataire" ? expediteurACrediter :
        null; // Ajoutez d'autres cas selon votre logique
  
      if (newStatus === "Livré" && !beneficiareDuCredit) {
        throw new Error("Impossible de déterminer qui créditer");
      }
  
      await updateLivraisonStatus(commande.id, newStatus);
      setCommande(prev => ({ ...prev, status: newStatus }));
  
      if (newStatus === "Livré" && beneficiareDuCredit && montant > 0) {
        console.log(`Crédit de ${montant}€ à ${beneficiareDuCredit}`);
        ...
        Alert.alert('Succès', `Livraison confirmée (${montant}€ crédités)`);
        return;
      }
