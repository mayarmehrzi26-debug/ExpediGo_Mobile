...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  const handleStartConversation = async (userType: 'expediteur' | 'destinataire') => {
    try {
      ...
      if (userType === 'expediteur') {
        if (!commande.expediteurId) {
          Alert.alert("Erreur", "ID expéditeur manquant");
          return;
        }
        recipientId = commande.expediteurId;
        recipientName = commande.createdBy?.name || 'Expéditeur';
      } else {
        if (!destinataire?.id) {
          Alert.alert("Erreur", "Destinataire non trouvé");
          return;
        }
        recipientId = destinataire.id;
        recipientName = commande.client?.name || 'Destinataire';
      }
  ...
  <>
    <TouchableOpacity 
      ...
      onPress={() => handleStartConversation('expediteur')}>
          ...
          <View style={styles.contactCard}>
            <View style={styles.contactInfo}>
              ...
              {commande.client?.phone && (
                <Text style={styles.contactPhone}>{commande.client.phone}</Text>
              )}
            </View>
            ...
            <View style={styles.contactActions}>
              {commande.client?.phone && (
                <>
                  <TouchableOpacity 
                    style={styles.iconButton}
                    onPress={() => handleStartConversation(commande.client.id, commande.client.name)}>
                    <MaterialIcons name="message" size={24} color="#44076a" />
                  </TouchableOpacity>
