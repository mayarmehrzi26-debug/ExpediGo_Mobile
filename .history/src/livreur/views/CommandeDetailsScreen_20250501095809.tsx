...
import { Phone, Message } from '@expo/vector-icons/MaterialIcons';
...
const CommandeDetailsScreen = ({ route }) => {
  ...
  }, [commande.status]); // Réabonnement quand le statut change
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      if (success) {
        ...
        // Gérer l'affichage des trajets en fonction du nouveau statut
        if (newStatus === "Picked") {
      ...
      {commande.createdBy?.phone && (
        <>
          <TouchableOpacity 
            ...
            <Phone name="phone" size={24} color="#4CAF50" />
          </TouchableOpacity>
          
          <TouchableOpacity 
            style={styles.contactButton} 
            onPress={() => handleSMS(commande.createdBy.phone)}>
            <Message name="message" size={24} color="#2196F3" />
          </TouchableOpacity>
        </>
      )}
    </View>
    ...
    <View style={styles.contactActions}>
      {commande.client?.phone && (
        <>
          <TouchableOpacity 
            style={styles.contactButton} 
            onPress={() => handleCall(commande.client.phone)}>
            <Phone name="phone" size={24} color="#4CAF50" />
          </TouchableOpacity>
