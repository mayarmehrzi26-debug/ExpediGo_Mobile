...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  const [commande, setCommande] = useState({
    ...initialCommande,
   totalAmount: initialCommande.totalAmount || 0,
  payment: initialCommande.payment || 'non_payé'
  ...
  const checkProximity = (location) => {
    ...
    // Détection arrivée à l'origine (pour pickup)
    ...
    // Détection arrivée à destination (pour livraison)
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      // Gestion spécifique pour les livraisons avec paiement "percevoir"
      if (newStatus === "Livré" && commande.payment === "percevoir") {
        if (!commande.createdBy?.id || !commande.totalAmount) {
          throw new Error("Données manquantes pour le crédit");
        }
        ...
        Alert.alert(
          'Succès', 
          `Livraison confirmée et ${commande.totalAmount}€ crédités à l'expéditeur`
        );
      }
    } catch (error) {
 ...
 const handleMainAction = () => {
  switch (commande.status) {
    ...
    case "En cours de pickup":
      // Le pickup se termine automatiquement quand on arrive à l'origine
      // via checkProximity
      break;
    case "Picked":
      ...
      break;
    case "En cours de livraison":
      // La livraison se termine automatiquement quand on arrive à destination
      // via checkProximity
      break;
  }
};
