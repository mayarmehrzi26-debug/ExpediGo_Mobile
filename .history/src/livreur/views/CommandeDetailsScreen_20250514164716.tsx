...
import { firebaseAuth ,firebasestorr} from '../../../FirebaseConfig';
...
import { addDoc, collection, doc, getDoc, getDocs, increment, limit, onSnapshot, orderBy, query, serverTimestamp, updateDoc, where, writeBatch } from "firebase/firestore";
...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  const [livreurLocation, setLivreurLocation] = useState(null); // Nouvel état pour la position du livreur
  ...
  const [livreurData, setLivreurData] = useState(null); // Données du livreur
            ...
            (location) => {
              if (isMounted) {
                ...
                if (firebaseAuth.currentUser?.uid) {
                  const livreurLocationRef = doc(firebasestore, "livreurLocations", firebaseAuth.currentUser.uid);
                  updateDoc(livreurLocationRef, {
                    latitude: location.coords.latitude,
                    longitude: location.coords.longitude,
  ...
  const calculateRegion = () => {
    ...
    if (livreurLocation) points.push(livreurLocation);
  ...
  const zoomToMarkers = () => {
    ...
    if (livreurLocation) coordinates.push(livreurLocation);
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      if (newStatus === "Livré" && expediteurId) {
        if (montant > 0) {
          console.log(`Tentative de crédit de ${montant}dt à l'expéditeur ${expediteurId}`);
      ...
      >
        ...
        <View style={styles.mapContainer}>
          ...
          >
            ...
            {/* Marqueur pour la position du livreur */}
            {livreurLocation && (
              <Marker
                coordinate={livreurLocation}
                title={`Position du livreur ${livreurData?.name || ''}`}
                description={`Dernière mise à jour: ${livreurLocation.timestamp?.toLocaleTimeString() || 'inconnue'}`}
                pinColor="#EA4335"
              >
                <MaterialIcons name="delivery-dining" size={30} color="#EA4335" />
              </Marker>
            )}
            ...
            {/* Trajet vers l'origine */}
            {showOriginRoute && livreurLocation && hasValidOrigin && (
              <Polyline
                coordinates={[
                  livreurLocation,
                  { latitude: commande.originLat, longitude: commande.originLng },
            ...
            {showDestinationRoute && livreurLocation && hasValidDestination && (
              <Polyline
                coordinates={[
                  livreurLocation,
                  { latitude: commande.destinationLat, longitude: commande.destinationLng },
