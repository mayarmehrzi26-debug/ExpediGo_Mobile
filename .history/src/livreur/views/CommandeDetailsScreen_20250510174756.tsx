...
const CommandeDetailsScreen = ({ route, navigation }) => {
  ...
  const handleStartConversation = async (userId, userName) => {
    try {
      ...
      // Vérifie l'existence de l'utilisateur dans users OU clients
      const userExists = await livraisonModel.checkUserExists(userId);
      if (!userExists) {
        Alert.alert("Erreur", "Le contact n'existe pas dans le système");
        return;
      }
  ...
  // Fonction pour oconst handleStartConversation = async (userId, userName) => {
  try {
    const currentUser = firebaseAuth.currentUser;
    if (!currentUser) {
      Alert.alert("Erreur", "Vous devez être connecté pour envoyer des messages");
  ...
  {commande.client?.phone && (
    <>
      <TouchableOpacity 
        style={styles.iconButton}
        onPress={() => destinataire ? 
          handleStartConversation(destinataire.id, commande.client.name) : 
          Alert.alert("Info", "Chargement des informations du destinataire...")
        }>
        <MaterialIcons name="message" size={24} color="#44076a" />
      </TouchableOpacity>
