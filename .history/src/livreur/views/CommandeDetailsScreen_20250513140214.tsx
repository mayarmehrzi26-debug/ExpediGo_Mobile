...
const checkProximity = (location) => {
  ...
  setUpdating(true);
  try {
    // Rechargez les données directement depuis Firestore
    const commandeActuelle = await livraisonModel.getCommandeById(commande.id);
    const montant = commandeActuelle.totalAmount;

    console.log('Détails complets de la commande:', commandeActuelle); // Ajouté pour debug

    if (!commandeActuelle) {
      throw new Error("Commande introuvable");
    }

    // Vérification renforcée de l'expéditeur
    const expediteurId = commandeActuelle.createdBy?.id 
                      || commandeActuelle.clientId 
                      || commandeActuelle.client?.id;
    ...
    if (newStatus === "Livré" && expediteurId) {
      if (montant > 0) {
        console.log(`Tentative de crédit de ${montant}€ à ${expediteurId}`);
        await livraisonModel.creditExpediteur(
          expediteurId, 
          montant,
          commande.id
        );
        Alert.alert('Succès', `Livraison confirmée (${montant}€ crédités à ${expediteurId})`);
      } else {
    ...
    console.error("Erreur complète:", {
      error: error.message,
      stack: error.stack,
      commande: commande // Ajout des données de la commande dans le log
    });
