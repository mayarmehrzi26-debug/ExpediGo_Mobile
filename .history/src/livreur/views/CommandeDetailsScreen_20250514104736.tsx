...
useEffect(() => {
  if (commande.status === "En cours de pickup" || commande.status === "En cours de livraison") {
    const currentUser = firebaseAuth.currentUser;
    if (currentUser) {
      livraisonModel.startTrackingLivraison(commande.id, currentUser.uid);
    }
  }

  return () => {
    if (commande.status === "Livré" || commande.status === "Retour") {
      livraisonModel.stopTrackingLivraison(commande.id);
    }
  };
}, [commande.status]);
