  ...
  const getMapRegion = () => {
    if (hasValidOrigin && hasValidDestination) {
      return {
        ...
        latitudeDelta: Math.abs(commande.originLat - commande.destinationLat) * 2 + 0.1,
        longitudeDelta: Math.abs(commande.originLng - commande.destinationLng) * 2 + 0.1,
      };
    }
...
const DetailRow = ({ icon, label, value, onPress }) => (
  <TouchableOpacity style={styles.detailRow} onPress={onPress} disabled={!onPress}>
    ...
    <Text style={styles.detailValue} numberOfLines={2}>{value || 'Non spécifié'}</Text>
