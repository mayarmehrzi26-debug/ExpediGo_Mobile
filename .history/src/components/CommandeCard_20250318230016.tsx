...
import { collection, doc, getDocs, query, Timestamp, updateDoc, setDoc, getDoc } from 'firebase/firestore';
  ...
  const handlePaymentMethodSelection = async (method: string) => {
    if (selectedOrder) {
      try {
        ...
        // Récupérer le montant du produit
      ...
      {selectedOrder && (
        ...
        >
          <View style={styles.modalView}>
            ...
            {statuses.map((status) => (
              <TouchableOpacity key={status.id} onPress={() => handlePaymentMethodSelection(status.name)}>
