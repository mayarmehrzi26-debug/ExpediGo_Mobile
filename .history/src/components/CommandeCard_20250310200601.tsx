...
import { collection, doc, getDocs,getDoc, query, updateDoc } from 'firebase/firestore';
  ...
  const fetchOrders = async () => {
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        return {
          id: numericId,
          origin: data.origin || "origine inconnu",
  ...
  const updateOrderStatus = async (newStatus: string) => {
    if (selectedOrder) {
      try {
        ...
        // Vérifiez si le document existe avant de le mettre à jour
        ...
        if (orderSnapshot.exists()) {
          ...
          console.error("Le document n'existe pas :", selectedOrder.id);
        }
      } catch (error) {
