...
import sendEmail from 'react-native-email'; // Importer react-native-email
...
interface Order {
  ...
  date?: string;
  status?: string;
  clientEmail?: string; // Ajouter l'email du client
}
...
const OrderCard: React.FC = () => {
    ...
    if (selectedOrder) {
      try {
        ...
        if (selectedOrder.clientEmail) {
          sendEmailToClient(selectedOrder.clientEmail, selectedOrder.id);
        }
      } catch (error) {
  ...
  const sendEmailToClient = (to: string, orderId: string) => {
    const subject = 'Votre colis est pris en charge';
    const body = `Votre colis est pris en charge. Vous pouvez suivre votre colis avec orderId=${orderId}.`;
