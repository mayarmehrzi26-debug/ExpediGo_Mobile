...
const handleConfirmDelivery = async () => {
    if (selectedOrder) {
      try {
        ...
        // 🔹 Générer le code QR avec l'ID de commande
        const qrCodeUrl = await QRCode.toDataURL(selectedOrder.id); // Générer un code QR sous forme d'URL
  
        // 🔹 Créer le message dynamique avec l'ID de commande et le code QR
        const emailTitle = "Confirmation de livraison"; // Titre de l'email
        const message = `Votre colis a été pris en charge. Vous pouvez suivre votre colis avec l'ID de commande : ${selectedOrder.id}. Voici un code QR pour un accès rapide : <img src="${qrCodeUrl}" alt="QR Code de suivi" />`;
  
        // 🔹 Envoyer l'email au client avec le message dynamique
        await sendEmail(emailTitle, clientEmail, message);
