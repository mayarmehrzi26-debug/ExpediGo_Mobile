...
import { collection, deleteDoc, doc, getDocs, query, Timestamp, updateDoc } from 'firebase/firestore';
...
const OrderCard: React.FC = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        const mailSnapshot = await getDocs(collection(firebasestore, "clients"));
        const mailData = clientSnapshot.docs.find(clientDoc => clientDoc.id === data.client)?.data();
        ...
        return {
          ...
          clientEmail: clientData?.email,  // Assurez-vous d'inclure l'email du client ici
        };
      }));
  ...
  const handleConfirmDelivery = async () => {
    if (selectedOrder) {
      try {
        // Mise à jour du statut de la commande dans Firestore
        const orderRef = doc(firebasestore, "livraisons", selectedOrder.id);
        await updateDoc(orderRef, { status: "Livrée" });

        // Envoi de l'e-mail via l'API Node.js
        if (selectedOrder.clientEmail) {
          const response = await fetch('http://192.168.199.160:5000/send-email', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({
              to: selectedOrder.clientEmail, // Email du client
              subject: 'Confirmation de livraison',
              text: `Le livreur a pris en charge votre colis. Votre numéro de commande est ${selectedOrder.id}. Vous pouvez suivre votre colis dans notre application.`,
            }),
          });

          const data = await response.json();
          if (response.ok) {
            console.log('E-mail envoyé:', data.message);
          } else {
            console.error('Erreur lors de l\'envoi de l\'email:', data.error);
          }
        }

        // Suppression de la commande après la confirmation
