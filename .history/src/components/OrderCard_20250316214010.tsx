    ...
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        return {
          ...
          clientEmail: clientData?.email || "email-inconnu@example.com", // Ajoutez l'e-mail du client
        };
      }));
 ...
 const handleConfirmDelivery = async () => {
  if (selectedOrder) {
    try {
      ...
      const clientEmail = "mayamehrzi8@gmail.com"; // Remplacez par l'e-mail du client
      await sendEmailToClient(clientEmail);
  ...
  try {
    const response = await fetch('http://localhost:5000/send-email', {
      ...
      body: JSON.stringify({
        to: clientEmail, // Remplacez par l'e-mail du client
        subject: 'Confirmation de livraison',
        text: 'Votre commande a été livrée avec succès. Merci pour votre confiance !',
      }),
    });
