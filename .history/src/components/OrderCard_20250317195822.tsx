...
const OrderCard: React.FC = () => {
    ...
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        return {
          ...
          clientEmail: clientData?.email || "", // Assurez-vous d'obtenir l'email du client
        };
      }));
  ...
  const sendEmail = async (to: string, orderId: string) => {
    const apiKey = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..sHpKX4TF8Ja3ecg-KDiX7II30zj9L37jiNIyb83ndVqiw_ch50H0mE7ImP9hQMprnHk58NRtAy4hC248c_vzGre9mIoTJUtaYh1e95fEELRHXklUQNIplFJJ9tHJ8qfGEuS02yG-2d9ffqdc8yJ_mvhGnMlnnxI4QEu6CHRxStrGmzfDXUjky3qWUQWZm8n3tFMbrAG6i__Z3h7sJ6IMI_SwCX0T7J0MMswvFzbKlVIvmbbRdVeCLBKeeN5-HnyT-vTGSRG1dJdAxygII1EcKlSiylwFqkYAhGwBuPzLQT_kVaa58uVQFKi0M_-hqugKJp07cuEVNeRW0DLVpkFdRCVAFDLh8bQYjpekBkbxDycq47JjXZW-GJTIPaUgF5RNube2jCfrno_aUQvgB0yv4lzFeyMn5ObdoWBR0hHbh_UkNNvIKAfZHA_X5AmYoZ2lta0jZDd6Ayy0PxVx-HX1FAzmdZ-SjRWe_pcACHpvzrhp3yXr801Aq1DbCkP_Y7xRb9fswJkJRDGD8w2ftf7TKMpaU7wGNg-wCSi-KbDg2xRNnIvsIwg6miaihSaFGxrfNtfRpBPIGuwsSZCbF-31vxzZtluMBAHts1WnpbSn_Y4WTuosTwB2PZ-amUBzBLHr5CSSgTO2wiN9RN797YVl-lm-o4kTzeB5vxJSFPwHwOg'; // Remplacez par votre clé API MailerLite
    const url = 'https://api.mailerlite.com/api/v2/email/send';
  
    const emailData = {
      to: [{ email: to }],
      subject: 'Votre colis est pris en charge',
      body: `Votre colis est pris en charge. Vous pouvez suivre votre colis avec orderId=${orderId}.`,
    };
  ...
  const handleConfirmDelivery = async () => {
    if (selectedOrder) {
      try {
        ...
        // Copier le document de "livraisons" à "Orders"
        ...
        // Supprimer la commande de "livraisons" après confirmation
        await deleteDoc(orderRef);

        // Envoyer un e-mail au client
        await sendEmail(selectedOrder.clientEmail, selectedOrder.id);

        // Mettre à jour l'état local pour supprimer la commande confirmée
