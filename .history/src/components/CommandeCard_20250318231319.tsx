...
interface Order {
  ...
  date?: string;
  status?: string;
  productId?: string; // Ajouter productId
}
...
const CommandeCard: React.FC = () => {
    ...
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        return {
          ...
          productId: data.productId, // Ajouter le productId
        };
      }));
  ...
  const updateSolde = async (amount: number) => {
    try {
      // Récupérer le document de solde existant
      ...
      if (soldeDoc.exists()) {
        ...
        // Créer un nouveau document de solde
  ...
  const updateOrderStatus = async (newStatus: string) => {
    if (selectedOrder) {
      try {
        const orderRef = doc(firebasestore, "Orders", selectedOrder.id);
  ...
  const handlePaymentMethodSelection = async (method: string) => {
    if (selectedOrder) {
      try {
        ...
        // Récupérer le montant du produit associé à la commande
        const productSnapshot = await getDocs(collection(firebasestore, "products"));
        const productData = productSnapshot.docs.find((doc) => doc.id === selectedOrder.productId)?.data();
        const amount = productData?.amount || 0;
