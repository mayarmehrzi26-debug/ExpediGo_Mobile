...
const OrderCard: React.FC = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        // Fetch the address data // Fetch the client data
        ...
        const addressData = addressSnapshot.docs.find(addressDoc => addressDoc.id === data.address)?.data();
         ...
         // Fetch the email 
         const emailSnapshot = await getDocs(collection(firebasestore, "clients"));
         const emailData = emailSnapshot.docs.find(emailData => emailData.id === data.email)?.data();
        ...
        return {
          ...
          clientEmail: emailData?.email || "Email inconnu", // Fetching the email from client data
        };
      }));
  ...
  const sendEmailNotification = (clientEmail: string, orderId: string) => {
    ...
    emailjs.send('service_5eu5z5n', 'template_ola4v6e', templateParams, {
      publicKey: 'YzX4Plt99kkGq5oY4', // Your User ID
      privateKey: 'ctVLmKQb3u5WjGOnl-Rk-' // Your Private Key
  }).then((response) => {
