...
const OrderCard: React.FC = () => {
    ...
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        return {
          ...
          clientEmail: clientData?.email || "email inconnu", // Assurez-vous que l'email est chargé
        };
      }));
  ...
  const handleConfirmDelivery = async () => {
    if (selectedOrder) {
      try {
        // Référence à la commande actuelle dans "livraisons"
        ...
        // Copier le document de "livraisons" vers "Orders"
        ...
        const clientEmail = selectedOrder.clientEmail; // Assurez-vous d'avoir l'e-mail du client
        ...
        // Ajoutez les console.log ici
        console.log("Sending email to:", clientEmail);
        console.log("Template params:", templateParams);
  
        emailjs.send('service_5eu5z5n', 'template_ci991a7', {
          to_email: 'mayarmehrzi22@gmail.com',
          order_id: '12345',
          origin: 'Tunis',
          destination: 'Sousse',
          date: '2025-03-17 12:28 AM'
        }, 'YzX4Plt99kkGq5oY4')
        .then(response => console.log('Email envoyé avec succès', response))
        .catch(error => console.error('Erreur lors de l’envoi de l’email', error));
          
        // Mettre à jour l'état local pour retirer la commande confirmée
