...
interface StatusBadgeProps {
  deliveryId: string; // ID de la livraison pour récupérer le statut
}
...
const StatusBadge: React.FC<StatusBadgeProps> = ({ deliveryId }) => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const statusDoc = await getDoc(doc(firebasestore, "Status", deliveryId)); // Utiliser deliveryId passé en prop
        if (statusDoc.exists()) {
          const statusData = statusDoc.data();
          console.log("Données du statut :", statusData); // Debug: afficher les données récupérées
          
          // Vérifiez si le champ status existe et est une chaîne
          if (statusData && typeof statusData.nomStat === 'string') {
            setStatusText(statusData.status); // Utiliser le champ approprié pour le statut
          } else {
