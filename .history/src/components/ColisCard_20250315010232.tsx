  ...
  const fetchOrders = async () => {
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        const productData = productSnapshot.docs.find(productData => productData.id === data.name)?.data();
        ...
        return {
          id: doc.id,
          product: productData?.product|| "origine inconnu",
          ...
          <View key={order.id} style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <View style={styles.cardDetails}>
                ...
                <Text style={styles.orderNumber}>Produit :{order.product}</Text>
