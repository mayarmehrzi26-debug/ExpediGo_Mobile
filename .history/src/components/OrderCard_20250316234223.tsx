  ...
  const handleConfirmDelivery = async () => {
    if (selectedOrder) {
      try {
        ...
        // Envoi de l'email après confirmation 🚀
        fetch("http://localhost:3000/send-email", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            to: "mayarmehrzi22@gmail.com",
            subject: "Confirmation de livraison",
            text: `Votre commande ${selectedOrder.id} a été expédiée de ${selectedOrder.origin} vers ${selectedOrder.destination}.`,
          }),
        })
