...
import { StackNavigationProp } from '@react-navigation/stack'; // Importez StackNavigationProp
...
interface OrderCardProps {
  statusFilter?: string; // Prop pour filtrer par statut
  navigation: StackNavigationProp<RootStackParamList, 'PackageDetails'>; // Prop pour la navigation
}

const OrderCard: React.FC<OrderCardProps> = ({ statusFilter, navigation }) => {
    ...
    if (selectedOrder) {
      try {
        ...
        await updateDoc(orderRef, {
          status: "En attente d'enlèvement", // Ou tout autre statut approprié
    ...
    <View>
      {filteredOrders.length > 0 ? (
        filteredOrders.map((order) => (
          <TouchableOpacity
            key={order.id}
            onPress={() => navigation.navigate('PackageDetailss', { orderId: order.id })} // Naviguer vers PackageDetails avec l'ID de la commande
          >
        ...
        <Text style={styles.noOrders}>Aucune commande effectuée</Text>
      )}
