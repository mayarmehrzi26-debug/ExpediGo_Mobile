...
const StatusBadge: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const statusDoc = await getDoc(doc(firebasestore, "livraisons", "0")); // Récupérer le document avec l'ID 0
        if (statusDoc.exists()) {
          const statusData = statusDoc.data();
          setStatusText(statusData.nomStat); // Remplacer par le champ approprié
        } else {
