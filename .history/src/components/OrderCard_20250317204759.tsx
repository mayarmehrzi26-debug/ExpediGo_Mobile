...
import { Buffer } from 'buffer'; // Pour encoder la clé API en Base64
...
const sendEmail = async (to: string, subject: string, body: string) => {
  const apiKey = '0f6f1a427850cf47a28449859635bd39-3d4b3a2a-24b9924e'; // Remplacez par votre clé API Mailgun
  const domain = 'sandbox80ee7cc6b001492f89014e31d24aaf4b.mailgun.org'; // Remplacez par votre domaine vérifié (ex: sandbox12345.mailgun.org)
  const url = `https://api.mailgun.net/v3/${domain}/messages`;

  const formData = new FormData();
  formData.append('from', 'mayarmehrzi22@gmail.Com'); // Remplacez par votre adresse e-mail
...
const handleConfirmDelivery = async () => {
  if (selectedOrder) {
    try {
      // Référence à la commande dans "livraisons"
      const orderRef = doc(firebasestore, "livraisons", selectedOrder.id.toString());

      // Référence au nouveau document dans "Orders"
      ...
      if (selectedOrder.clientEmail) {
        await sendEmail(
          ...
          `Votre colis est pris en charge. Vous pouvez suivre votre colis avec orderId=${selectedOrder.id}.`
        );
      }
    } catch (error) {
