...
const OrderCard: React.FC = () => {
  ...
  const sendEmail = async (to, orderId) => {
    const apiKey = 'dad0b28c-7e15-4e8c-916d-27c8c1feb66e'; // Remplacez par votre clé API Postmark
    const url = 'https://api.postmarkapp.com/email';
  
    const emailData = {
      From: 'mayarmehrzi22@expedigo.com', // Remplacez par votre adresse d'expédition
      To: to,
      Subject: 'Votre colis est pris en charge',
      TextBody: `Votre colis est pris en charge. Vous pouvez suivre votre colis avec orderId=${orderId}.`,
    };
  ...
  const handleConfirmDelivery = async () => {
    if (selectedOrder) {
      try {
        ...
        const clientEmail = selectedOrder.clientEmail; // Récupérer l'email du client
