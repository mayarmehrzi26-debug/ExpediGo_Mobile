    ...
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        return {
          ...
          clientEmail: clientData?.email || "", // Ensure client email is fetched
        };
      }));
  ...
  const handleConfirmDelivery = async () => {
    if (selectedOrder) {
      try {
        ...
        const clientEmail = orderData.clientEmail; // Make sure this is populated
  
        // Add logging to check the client email
        console.log("Client Email:", clientEmail);
  
        if (!clientEmail) {
          console.error("Client email is empty!");
          return; // Prevent sending email if the email is empty
        }
