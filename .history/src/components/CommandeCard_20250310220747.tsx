...
import { collection, doc, getDocs, query, Timestamp, updateDoc } from 'firebase/firestore'; // Importation correcte
    ...
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        return {
          ...
          date: formattedDate, 
         status: data.nomStat || "En attente", // Ajouter un champ "status"
        };
      }));
