...
import { CashPresenter } from '../expediteur/caisse/presenter/CashPresenter';
...
const CashBox: React.FC = () => {
  ...
  const requestWithdrawal = async () => {
    ...
    try {
      ...
      const { amount, method } = await showWithdrawalDialog(parseFloat(balance.enAttente));
  ...
  const formatAmount = (amount: string) => {
    return parseFloat(amount).toFixed(3).replace('.', ',');
  };
    ...
    <>
      <View style={styles.cashBox}>
        ...
        <View style={styles.cashRow}>
          ...
          <Text style={styles.cashValueBlue}>{formatAmount(balance.disponible)} {balance.currency}</Text>
        </View>
        <View style={styles.cashRow}>
          <Text style={styles.cashLabel}>Cash non Disponible</Text>
          <Text style={styles.cashValue}>{formatAmount(balance.enAttente)} dt</Text>
        </View>
        <View style={styles.divider} />
        <View style={styles.cashRow}>
          <Text style={[styles.cashLabel, styles.bold]}>Total</Text>
          <Text style={[styles.cashValue, styles.bold]}>{formatAmount(balance.total)} {balance.currency}</Text>
        </View>
                ...
                >
                  <Text style={styles.buttonText}>
                    {canRequestWithdrawal 
                      ? `Demander versement (${formatAmount(balance.enAttente)} dt)`
