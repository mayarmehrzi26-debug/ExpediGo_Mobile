  ...
  const fetchOrders = async () => {
    try {
      ...
      const deliveriesList = await Promise.all(querySnapshot.docs.map(async (doc) => {
        ...
        const productSnapshot = await getDocs(collection(firebasestore, "adresses"));
        const productData = productSnapshot.docs.find(productData => productData.id === data.address)?.data();
