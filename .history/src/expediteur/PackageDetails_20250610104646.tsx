      ...
      try {
        ...
        const fetchData = async () => {
          try {
            ...
            if (deliveryData.products && Array.isArray(deliveryData.products)) {
              productsData = await Promise.all(
                deliveryData.products.map(async (productItem: any) => {
                  const productDoc = await getDoc(doc(firebasestore, 'products', productItem.productId));
                  return {
                    id: productItem.productId,
                    name: productDoc.exists() ? productDoc.data()?.name : 'Produit inconnu',
            ...
            const [clientDoc, addressDoc, assignedToDoc] = await Promise.all([
