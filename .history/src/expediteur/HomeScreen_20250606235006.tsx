...
import { doc, updateDoc ,collection,getDocsgetDoc} from 'firebase/firestore';
...
const [notificationsVisible, setNotificationsVisible] = useState(false); // Nouvel état
  const [notificationsList, setNotificationsList] = useState([]); // Liste des notifications
 ...
 const handleShowNotifications = async () => {
    try {
      // Récupérer les notifications de l'utilisateur depuis Firestore
      const user = firebaseAuth.currentUser;
      if (user) {
        const userDoc = await getDoc(doc(firebasestore, 'users', user.uid));
        if (userDoc.exists()) {
          const userData = userDoc.data();
          // Supposons que vous stockez les notifications dans un sous-collection
  ...
  useEffect(() => {
    ...
    const responseSubscription = Notifications.addNotificationResponseReceivedListener(response => {
      navigation.navigate('TicketScreen', { 
        ticketId: response.notification.request.content.data.ticketId 
      });
    });
