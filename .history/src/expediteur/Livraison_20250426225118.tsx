...
const Livraison: React.FC = () => {
  ...
  const toggleSelectAll = () => {
    if (selectedCards.length === deliveries.length) {
      setSelectedCards({});
    } else {
      const allSelected = deliveries.reduce((acc, delivery) => {
        acc[delivery.id] = true;
        return acc;
      }, {});
  ...
  const exportSelectedDeliveries = () => {
    // Cette fonction est maintenant gérée dans FileExplorer
  };
  ...
  const toggleSelectAll = useCallback(() => {
    setSelectedCards(prev => {
      // Si toutes les livraisons sont déjà sélectionnées, on les désélectionne toutes
      if (Object.keys(prev).length === filteredDeliveries.length && 
          filteredDeliveries.every(d => prev[d.id])) {
        return {};
      }
      
      // Sinon, on sélectionne toutes les livraisons visibles (filtrées)
      const newSelection = {};
      filteredDeliveries.forEach(delivery => {
        newSelection[delivery.id] = true;
      });
      return newSelection;
    });
  }, [filteredDeliveries]);
  ...
  const selectedDeliveriesIds = Object.keys(selectedCards).filter(id => selectedCards[id]);
      ...
      <FileExplorer 
        ...
        exportSelectedDeliveries={exportSelectedDeliveries}/>   
