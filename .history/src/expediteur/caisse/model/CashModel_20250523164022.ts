...
async getTransactions(limitCount: number = 20): Promise<Transaction[]> {
  try {
    ...
    for (const docSnap of snapshot.docs) {
      ...
      if (data.clientId) {
        try {
          // Correct way to get a document reference
          const clientDocRef = doc(firebasestore, "clients", data.clientId);
          const clientDoc = await getDoc(clientDocRef);
          
          if (clientDoc.exists()) {
            const clientData = clientDoc.data();
            clientName = clientData?.name || clientData?.fullName || "Client inconnu";
          }
        } catch (error) {
          console.error("Error fetching client:", error);
          // Fallback to clientName from transaction data if available
