...
async getTransactions(limitCount: number = 20): Promise<Transaction[]> {
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      if (data.clientId) {
        try {
          const clientDoc = await getDoc(doc(firebasestore, "clients", data.clientId));
          if (clientDoc.exists()) {
            clientName = clientDoc.data().name || clientDoc.data().fullName || "Client inconnu";
          }
        } catch (error) {
      ...
      transactions.push({
        ...
        createdAt: createdAt,
        clientName: data.clientName || data.customerName || clientName,
