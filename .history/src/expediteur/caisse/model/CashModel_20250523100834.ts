...
async requestWithdrawal(amount: number, paymentMethod: "Espèces" | "Virement", transactionIds?: string[]): Promise<string> {
  try {
    console.log("[CashModel] Starting withdrawal request...");
    console.log(`User: ${this.userId}, Amount: ${amount}, Method: ${paymentMethod}`);
    ...
    const withdrawalRequestsRef = collection(firebasestore, "withdrawalRequests");
    ...
    console.log("[CashModel] Withdrawal data:", withdrawalData);

    // Ajouter le document
    const docRef = await addDoc(withdrawalRequestsRef, withdrawalData);
    console.log("[CashModel] Withdrawal request created with ID:", docRef.id);
    ...
    console.error("[CashModel] Error in requestWithdrawal:", error);
    throw error;
  }
}
