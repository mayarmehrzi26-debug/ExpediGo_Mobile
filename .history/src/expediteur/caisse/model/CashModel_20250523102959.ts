...
async requestWithdrawal(amount: number, paymentMethod: "Espèces" | "Virement", transactionIds?: string[]): Promise<string> {
  try {
    console.log("[CashModel] Starting withdrawal request with:", {
      userId: this.userId,
      amount,
      paymentMethod,
      transactionIds
    ...
    console.log("[CashModel] Current balance:", balance);

    if (amount <= 0) throw new Error("Montant invalide");
    if (amount > balance.enAttente) throw new Error("Solde insuffisant");

    // 2. Vérification des demandes existantes
    const hasPending = await this.hasPendingWithdrawal();
    if (hasPending) throw new Error("Demande en cours");
    ...
    console.log("[CashModel] Attempting to add document to withdrawalRequests...");
    
    // 4. Référence explicite à la collection
    const withdrawalRequestsCollection = collection(firebasestore, "withdrawalRequests");
    const docRef = await addDoc(withdrawalRequestsCollection, withdrawalData);
    
    console.log("[CashModel] Document created with ID:", docRef.id);
    ...
    console.log("[CashModel] Balance updated successfully");
    return docRef.id;

  } catch (error) {
    console.error("[CashModel] ERROR in requestWithdrawal:", {
      error: error.message,
      stack: error.stack
    });
    throw error;
