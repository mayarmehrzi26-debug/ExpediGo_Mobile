...
export class CashModel {
  ...
  async requestWithdrawal(amount: number, paymentMethod: "Espèces" | "Virement", transactionIds?: string[]): Promise<string> {
    try {
      ...
      if (amount > balance.enAttente) throw new Error("Solde en attente insuffisant");

      const hasPending = await this.hasPendingWithdrawal();
      if (hasPending) throw new Error("Vous avez déjà une demande en attente");
      ...
      if (transactionIds && transactionIds.length > 0) {
        withdrawalData.transactionIds = transactionIds;
      }
