...
async requestWithdrawal(amount: number, paymentMethod: "Espèces" | "Virement", transactionIds?: string[]): Promise<string> {
  try {
    console.log("[DEBUG] Creating withdrawal request with:", {
      userId: this.userId,
      amount,
      paymentMethod,
      transactionIds
    ...
    if (!this.userId) throw new Error("User ID manquant");
    if (typeof amount !== "number" || amount <= 0) throw new Error("Montant invalide");
    if (!["Espèces", "Virement"].includes(paymentMethod)) throw new Error("Méthode de paiement invalide");

    const withdrawalData = {
      userId: this.userId,
      amount,
      status: "pending", // Important: doit correspondre aux règles
      paymentMethod,
      transactionIds: transactionIds || [],
      createdAt: serverTimestamp(),
      _debug: new Date().toISOString() // Pour le débogage
    };

    console.log("[DEBUG] Data being sent:", withdrawalData);

    const docRef = await addDoc(collection(firebasestore, "withdrawalRequests"), withdrawalData);
    console.log("[SUCCESS] Withdrawal created with ID:", docRef.id);

    return docRef.id;
  } catch (error) {
    console.error("[ERROR] Full error details:", {
      message: error.message,
      code: error.code, // Code d'erreur Firestore
      stack: error.stack
    });
    throw error;
  }
