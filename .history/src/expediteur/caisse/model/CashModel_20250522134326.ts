...
async getBalance(): Promise<Balance> {
  ...
  try {
    const q = query(
      collection(firebasestore, "transactions"),
      where("userId", "==", this.userId),
      where("status", "==", "Non traité"), // Ou le statut que vous utilisez pour "non traité"
      orderBy("createdAt", "desc")
    );
...
async getUntreatedTransactions(): Promise<PendingTransaction[]> {
  try {
    const q = query(
      collection(firebasestore, "transactions"),
      where("userId", "==", this.userId),
      where("status", "==", "non_traite"), // Statut initial
      orderBy("createdAt", "desc")
    );
    ...
    console.error("Error getting untreated transactions:", error);
    throw error;
  }
}
