...
const CashView = () => {
        ...
        const newPresenter = new CashPresenter(
          {
            ...
            displayPendingTransactions: setPendingTransactions,
            showTransactionSelectionDialog: showTransactionSelectionDialog
          },
          user.uid
        );
  ...
  const showTransactionSelectionDialog = (transactions: Transaction[]): Promise<string[]> => {
  ...
  const renderTransactionItem = ({ item }: { item: Transaction }) => (
    <TouchableOpacity
      style={[
        ...
        selectedTransactions.includes(item.id) && styles.selectedTransactionItem
      ]}
    ...
    <View style={styles.requestCard}>
      <View style={styles.requestHeader}>
        ...
        <View style={[styles.statusBadge, { 
          backgroundColor: request.status === 'approved' ? '#54E598' : 
                         request.status === 'rejected' ? '#FF5252' : '#FF9800'
        }]}>
      ...
      <View style={styles.requestDetails}>
        ...
        <Text style={styles.requestDate}>
          {request.createdAt?.toLocaleDateString?.() || 'Date inconnue'} à{' '}
          {request.createdAt?.toLocaleTimeString?.([], {hour: '2-digit', minute:'2-digit'}) || ''}
        </Text>
      </View>

      {/* Affichage amélioré de la raison de rejet */}
      {request.status === 'rejected' && (
        <View style={styles.rejectionContainer}>
          <Text style={styles.rejectionTitle}>
            {request.rejectionReason ? 'Raison du rejet:' : 'Demande rejetée sans raison spécifiée'}
          </Text>
  ...
  const TransactionCard = ({ transaction }: { transaction: Transaction }) => (
    <View style={styles.transactionCard}>
      ...
      <View style={styles.clientRow}>
        ...
        <Text style={styles.clientName}>{transaction.clientName || "Non spécifié"}</Text>
      </View>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalFooter}>
              <Text style={styles.totalText}>
                Total sélectionné: {pendingTransactions
