...
const CashView = () => {
  ...
  if (!userId || !presenter) return;

  try {
    const cashModel = new CashModel(userId);
    
    // Vérifier qu'il n'y a pas déjà une demande en cours
    const hasPending = await cashModel.hasPendingWithdrawal();
    if (hasPending) {
    ...
    const pending = await cashModel.getPendingTransactions();
    if (pending.length === 0) {
      Alert.alert("Info", "Aucune transaction disponible pour versement");
      return;
    }
    ...
    Alert.alert("Erreur", error.message || "Échec de la préparation");
  }
};
