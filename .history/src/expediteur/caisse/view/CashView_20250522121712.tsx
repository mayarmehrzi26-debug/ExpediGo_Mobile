...
const handleRequestWithdrawal = async () => {
  try {
    ...
    if (pendingAmount <= 0) {
      Alert.alert("Erreur", "Aucun solde disponible pour versement");
      return;
    }

    const { method } = await showWithdrawalDialog(pendingAmount);
    
    // Utilisez directement le modèle au lieu du presenter
    const cashModel = new CashModel(userId);
    await cashModel.requestWithdrawal(pendingAmount, method);
    
    Alert.alert("Succès", `Demande de ${pendingAmount}dt envoyée`);
