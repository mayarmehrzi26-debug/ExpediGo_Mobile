...
const [hasPendingRequest, setHasPendingRequest] = useState(false);
  ...
  // Boîte de dialogue de sélection des transactions
 const showTransactionSelectionDialog = (transactions: PendingTransaction[]): Promise<string[]> => {
  return new Promise((resolve) => {
    setPendingTransactions(transactions);
    setSelectedTransactions([]);
    setShowTransactionSelection(true);
    setResolveSelection(() => (ids) => {
      // Ne retourner que le premier ID sélectionné
      resolve(ids.length > 0 ? [ids[0]] : []);
    });
  });
};

  // Boîte de dialogue pour choisir le mode de paiement
  ...
  onPress: async (amountText) => {
    ...
    if (amount <= 0 || amount > maxAmount) {
      Alert.alert("Erreur", `Le montant doit être entre 0 et ${maxAmount}`);
      return;
    }
  ...
  // Gestion de la sélection des transactions
  ...
  // Gestion de la demande de versement
  ...
  // Composant pour l'affichage d'une transaction dans la sélection
  const TransactionSelectionItem = ({ 
    ...
    >
      ...
      {isSelected && <Entypo name="check" size={20} color="#54E598" />}
    </TouchableOpacity>
  );

  // Composant pour le badge de méthode de paiement
  ...
  // Composant pour la carte de solde
  const BalanceCard = () => (
...
>
  <Text style={styles.ButtonPaymntText}>
    {`Demander versement (${balance.enAttente} ${balance.currency} disponible)`}
  </Text>
  {!canRequestWithdrawal && (
    <Text style={{fontSize: 10, color: 'white'}}>
      {hasPendingRequest ? "Une demande est déjà en cours" : "Solde insuffisant"}
    </Text>
  )}
</TouchableOpacity>
    </View>
  ...
  // Composant pour une demande de retrait
...
const TransactionCard = ({ transaction }: { transaction: Transaction }) => (
  <View style={styles.transactionCard}>
    ...
    {/* Nouvelle ligne pour le nom du client */}
    ...
    <View style={styles.transactionDetails}>
      <View style={styles.statusContainer}>
        ...
        {transaction.status === "En attente" && <Entypo name="dot-single" size={12} color="orange" />}
        {transaction.status === "Non traité" && <Entypo name="dot-single" size={12} color="gray" />}
    ...
    const unsubscribe = onAuthStateChanged(firebaseAuth, (user) => {
      if (user) {
        ...
        const newPresenter = new CashPresenter({
          ...
          showTransactionSelectionDialog
        }, user.uid);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Modal de sélection des transactions */}
    <Modal
  animationType="slide"
  transparent={true}
...
>
  <View style={styles.modalContainer}>
    <View style={styles.modalContent}>
      <Text style={styles.modalTitle}>Sélectionnez une transaction</Text>
      ...
      <View style={styles.modalFooter}>
        <Text style={styles.totalText}>
          Montant sélectionné: {selectedTransactions.length > 0 
            ? pendingTransactions.find(t => t.id === selectedTransactions[0])?.amount.toFixed(3) 
            : "0.000"} dt
        </Text>
