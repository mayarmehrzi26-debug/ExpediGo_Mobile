...
const handleRequestWithdrawal = async () => {
  ...
  try {
    const cashModel = new CashModel(userId);
    
    // 1. Récupérer les transactions non traitées
    const untreatedTransactions = await cashModel.getUntreatedTransactions();
    
    if (untreatedTransactions.length === 0) {
      Alert.alert("Info", "Aucune transaction non traitée disponible");
      return;
    }

    // 2. Afficher la sélection
    setPendingTransactions(untreatedTransactions);
...
onPress={async () => {
  ...
  if (selectedTransactions.length > 0) {
    try {
      ...
      // 1. Marquer comme "en attente"
      await cashModel.markTransactionsAsPending(selectedTransactions);
      
      // 2. Créer la demande
      await cashModel.requestWithdrawal(total, "Virement"); // Ou méthode choisie
      
      Alert.alert("Succès", `Demande de ${total.toFixed(3)} dt envoyée`);
      
      // 3. Actualiser
      if (presenter) await presenter.loadData();
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal
        ...
        onRequestClose={() => setShowTransactionSelection(false)}
      >
  ...
  onPress={async () => {
    ...
    if (selectedTransactions.length > 0) {
      try {
        ...
        const { method } = await showWithdrawalDialog(total);
        
        // 1. Marquer les transactions comme en attente
        await cashModel.markTransactionsAsPending(selectedTransactions);
        
        // 2. Créer la demande de versement
        await cashModel.requestWithdrawal(total, method);
                  ...
                  onPress={async () => {
                    ...
                    if (selectedTransactions.length > 0) {
                      try {
                        ...
                        // Here you would call the actual methods to process the transactions
                        // For now, we'll just show an alert
                        Alert.alert("Succès", `Demande de versement de ${total.toFixed(3)} dt par ${method} créée`);
