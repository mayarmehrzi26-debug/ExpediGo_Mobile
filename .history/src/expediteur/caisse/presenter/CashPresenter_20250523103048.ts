...
async prepareWithdrawal(): Promise<void> {
  try {
    ...
    if (pendingTransactions.length === 0) {
      this.view.showError("Aucune transaction disponible");
      return;
    }

    console.log("[3/6] Showing transaction selection dialog...");
    ...
    console.log("[6/6] Processing withdrawal...");
    ...
    console.log("Withdrawal successful! Request ID:", requestId);
    ...
    if (error.message !== "User cancelled") {
      this.view.showError(`Échec: ${error.message}`);
    }
  }
}
