...
export class CashPresenter {
 ...
 async loadData(): Promise<void> {
  try {
    ...
    // Ne vérifiez pas hasPendingWithdrawal ici
    const canWithdraw = balance.enAttente > 0;
    
    console.log(`Withdrawal enabled: ${canWithdraw}, Pending balance: ${balance.enAttente}`);
...
async prepareWithdrawal(): Promise<void> {
  try {
    ...
    const untreatedTransactions = await this.model.getUntreatedTransactions();
    ...
    const selectedIds = await this.view.showTransactionSelectionDialog(untreatedTransactions);
    ...
    const totalAmount = untreatedTransactions
      .filter(t => selectedIds.includes(t.id))
      .reduce((sum, t) => sum + t.amount, 0);

    // Demander la méthode de paiement
    const method = await this.view.showPaymentMethodDialog();

    // Créer la demande de versement
    await this.model.requestWithdrawal(selectedIds, method);

    this.view.showSuccess(
      `Demande de versement (${method}) de ${totalAmount.toFixed(3)} dt envoyée`
    );
    await this.loadData();
  } catch (error) {
