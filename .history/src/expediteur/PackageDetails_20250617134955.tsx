...
const PackageDetails: React.FC<PackageDetailsProps> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchDeliveryDetails = async () => {
      try {
        ...
        if (deliveryData.status === "Echange") {
  ...
  const fetchExchangeDetails = async (commandeId: string) => {
    try {
      ...
      if (!snapshot.empty) {
        const exchangeData = snapshot.docs[0].data();
        console.log("Détails d'échange trouvés:", exchangeData);
        setExchangeDetails(exchangeData);
      } else {
        console.log("Aucun échange trouvé pour cette commande");
    ...
    <View style={styles.container}>
      ...
      <ScrollView>
        <View style={styles.card}>
          ...
          {(delivery.status === "Echange" && exchangeDetails) && (
            <TouchableOpacity 
