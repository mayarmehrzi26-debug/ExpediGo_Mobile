...
import { AjoutAdressView } from "../view/AjoutAdressView";
...
export const AdressPresenter: React.FC<AjoutAdressPresenterProps> = ({ navigation }) => {
  ...
  const initializeLocation = async () => {
    const hasPermission = await Adress.requestLocationPermission();
    if (!hasPermission) {
      Alert.alert("Permission refusée", "L'application a besoin d'accéder à votre localisation.");
      return;
    }

    const loc = await Adress.getCurrentLocation();
    setLocation(loc);

    const city = await Adress.getCityFromCoords(loc.latitude, loc.longitude);
    setZone(city);
  };
  ...
  const handleMapPress = async (event: any) => {
    ...
    const zoneName = await Adress.getCityFromCoords(latitude, longitude);
    setZone(zoneName);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      await AjoutAdress.saveAddress(zone, address, location);
