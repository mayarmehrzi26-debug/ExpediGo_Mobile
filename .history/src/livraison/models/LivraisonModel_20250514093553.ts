...
async creditExpediteur(expediteurId: string, montant: number, commandeId: string): Promise<void> {
  try {
    console.log(`[CREDIT] Début pour ${expediteurId}, montant: ${montant}`);

    // Validation
    if (!expediteurId) throw new Error("ID expéditeur manquant");
    if (typeof montant !== 'number' || montant <= 0) {
      console.log(`[CREDIT] Montant ${montant} non crédité (trop bas)`);
      return;
    }
    ...
    await batch.commit();
    console.log(`[CREDIT] Succès - ${montant}€ crédités à ${expediteurId}`);

  } catch (error) {
    console.error("[CREDIT] Erreur:", {
      error: error.message,
      expediteurId,
      montant,
      commandeId
