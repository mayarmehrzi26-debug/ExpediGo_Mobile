import { addDoc, collection, doc, getDoc, getDocs, increment, limit, onSnapshot, orderBy, query, serverTimestamp, setDoc, updateDoc, where, writeBatch } from "firebase/firestore";
...
export class LivraisonModel {
  ...
  listenToLivreurPosition(
    ...
    return onSnapshot(livreurLocationRef, (doc) => {
      if (doc.exists()) {
        const data = doc.data();
        if (data.latitude === 0 && data.longitude === 0) {
          console.log("Ignoring initial zero position");
          return;
        }
        callback({
          latitude: data.latitude,
        ...
        console.log("Document not found - triggering recreation");
        this.initializeLivreurPosition(livreurId)
          .then(() => console.log("Document recreated"))
  ...
  async updateOrCreateLivreurPosition(
    ...
    if (position.latitude === 0 && position.longitude === 0) {
      console.warn("Ignoring zero position update");
      return;
    }
    ...
    try {
      ...
      // Try to recreate document if update fails
      await this.initializeLivreurPosition(livreurId);
      throw error;
    }
  }
  async initializeLivreurPosition(livreurId: string): Promise<void> {
    const livreurLocationRef = doc(firebasestore, "livreurLocations", livreurId);
    const docSnap = await getDoc(livreurLocationRef);
