...
export class LivraisonModel {
  ...
  async getDestinataireByCommandeId(commandeId: string): Promise<any> {
    try {
      ...
      // Vérifie d'abord si le champ client est un document référence
      if (commandeData.client && typeof commandeData.client === 'object') {
        const clientDoc = await getDoc(commandeData.client);
        if (clientDoc.exists()) {
          return {
            id: clientDoc.id,
            ...clientDoc.data()
      ...
      if (commandeData.client && typeof commandeData.client === 'string') {
        // Essaye d'abord comme client
        const clientDoc = await getDoc(doc(firebasestore, "clients", commandeData.client));
        if (clientDoc.exists()) {
          return {
            id: clientDoc.id,
            ...clientDoc.data()
        ...
        // Sinon essaye comme utilisateur
        const userDoc = await getDoc(doc(firebasestore, "users", commandeData.client));
        if (userDoc.exists()) {
          return {
            id: userDoc.id,
            ...userDoc.data()
