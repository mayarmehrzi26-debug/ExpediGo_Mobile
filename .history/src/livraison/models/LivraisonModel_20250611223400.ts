...
async createExchangeRequest(
  ...
  try {
    ...
    // Utiliser une transaction batch pour les deux opérations
    const batch = writeBatch(firebasestore);

    // 1. Créer la demande d'échange
    const exchangeRef = doc(collection(firebasestore, "exchanges"));
    ...
    // 2. Mettre à jour le statut de la livraison
    const livraisonRef = doc(firebasestore, "livraisons", commandeId);
    ...
    await batch.commit();
    console.log("Demande d'échange créée et statut de livraison mis à jour");
