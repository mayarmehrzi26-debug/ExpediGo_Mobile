import { collection,or,query,where,orderBy, doc, getDoc, getDocs, setDoc } from "firebase/firestore";
...
async incrementClientDeliveries(clientId: string): Promise<void> {
  try {
    const clientRef = doc(firebasestore, "clients", clientId);
    const clientSnap = await getDoc(clientRef);
    
    if (clientSnap.exists()) {
      const currentTotal = clientSnap.data().total_deliveries || 0;
      await setDoc(clientRef, { total_deliveries: currentTotal + 1 }, { merge: true });
    }
  } catch (error) {
    console.error("Erreur lors de l'incrémentation des livraisons du client:", error);
    throw error;
  }
}
