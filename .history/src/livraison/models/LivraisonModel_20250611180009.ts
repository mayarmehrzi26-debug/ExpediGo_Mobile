  ...
  try {
    ...
    if (data.createdBy) {
      if (typeof data.createdBy === 'string') {
        // Si createdBy est un ID string
        expediteur = await this.getUserById(data.createdBy) || 
                    await this.getClientById(data.createdBy);
      } else if (data.createdBy.path) {
        // Si createdBy est une référence Firestore
        const expediteurDoc = await getDoc(doc(firebasestore, data.createdBy.path));
        if (expediteurDoc.exists()) {
          expediteur = { id: expediteurDoc.id, ...expediteurDoc.data() };
        }
      }
    }
    ...
    return {
      id: docSnap.id,
      ...data,
      createdBy: expediteur || data.createdBy, // Garde l'ID si on n'a pas trouvé l'expéditeur
      products,
      totalAmount: data.totalAmount || data.montant || this.calculateTotalAmount(products),
      payment: data.payment || data.paiement || 'Non spécifié'
    };
  } catch (error) {
