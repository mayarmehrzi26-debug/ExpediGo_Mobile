import { addDoc, collection, doc, getDoc, getDocs, increment, limit, orderBy, query, serverTimestamp, updateDoc, where, writeBatch } from "firebase/firestore";
...
export class LivraisonModel {
  ...
  async creditExpediteur(expediteurId: string, montant: number, commandeId: string): Promise<void> {
    try {
      console.log('[DEBUG] Début de creditExpediteur', {
        expediteurId,
        montant,
        commandeId,
      ...
      if (!expediteurId || !montant || montant <= 0 || !commandeId) {
        const errorMsg = `Paramètres invalides: expediteurId=${expediteurId}, montant=${montant}, commandeId=${commandeId}`;
      ...
      const expediteurRef = doc(firebasestore, "users", expediteurId);
      ...
      console.log('[DEBUG] Références créées', {
        expediteurPath: expediteurRef.path,
        soldePath: soldeRef.path,
        commandePath: commandeRef.path
      });
  
      // Vérification de l'existence des documents
      const [expediteurSnap, commandeSnap] = await Promise.all([
        getDoc(expediteurRef),
        getDoc(commandeRef)
      ]);
  
      console.log('[DEBUG] Résultats des vérifications', {
        expediteurExists: expediteurSnap.exists(),
        commandeExists: commandeSnap.exists()
      });
  
      if (!expediteurSnap.exists()) {
        const errorMsg = `Expéditeur non trouvé: ${expediteurId}`;
        console.error('[ERROR] ' + errorMsg);
        throw new Error(errorMsg);
      }
      if (!commandeSnap.exists()) {
        const errorMsg = `Commande non trouvée: ${commandeId}`;
        console.error('[ERROR] ' + errorMsg);
        throw new Error(errorMsg);
      }
  
      // Création ou mise à jour du solde
      const soldeSnap = await getDoc(soldeRef);
      console.log('[DEBUG] État du solde avant mise à jour', {
        soldeExists: soldeSnap.exists(),
        currentSolde: soldeSnap.exists() ? soldeSnap.data() : 'null'
      });
  
      if (!soldeSnap.exists()) {
        console.log('[DEBUG] Création d\'un nouveau solde');
        batch.set(soldeRef, {
          userId: expediteurId,
          soldeDisponible: 0,
        ...
        console.log('[DEBUG] Mise à jour du solde existant');
      ...
      console.log('[DEBUG] Mise à jour du statut de paiement de la commande');
      ...
      console.log('[DEBUG] Création d\'une nouvelle transaction');
      ...
      // Exécution du batch
      console.log('[DEBUG] Exécution du batch write');
      await batch.commit();
      console.log('[SUCCESS] Batch write exécuté avec succès');
  
      // Vérification après commit
      const soldeAfterUpdate = await getDoc(soldeRef);
      console.log('[DEBUG] État du solde après mise à jour', {
        soldeData: soldeAfterUpdate.exists() ? soldeAfterUpdate.data() : 'null'
      });
      
    } catch (error) {
      console.error('[ERROR] Échec du crédit:', {
        error: error.message,
        stack: error.stack,
        expediteurId,
        montant,
