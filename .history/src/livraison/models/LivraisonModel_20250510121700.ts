...
export class LivraisonModel {
  ...
  async getDestinataireByCommandeId(commandeId: string): Promise<any> {
    try {
      ...
      if (!commandeData.client) return null;
  
      // Vérifie d'abord dans la collection clients
      ...
      // Si pas trouvé, cherche dans users avec clientId correspondant
      const userQuery = query(
        collection(firebasestore, "users"),
        where("clientId", "==", commandeData.client)
      );
      const userSnapshot = await getDocs(userQuery);
