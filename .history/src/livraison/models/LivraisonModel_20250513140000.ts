...
export class LivraisonModel {
  ...
  async getCommandeById(commandeId: string): Promise<any> {
    try {
      ...
      // Extraction renforcée des informations client/expéditeur
      let clientInfo = {};
      
      // Cas 1: client est une référence DocumentReference
      if (data.client?.path) {
        const clientDoc = await getDoc(doc(firebasestore, data.client.path));
        clientInfo = clientDoc.exists() ? { clientId: clientDoc.id, ...clientDoc.data() } : {};
      } 
      // Cas 2: client est un ID string
      else if (typeof data.client === 'string') {
        clientInfo = { clientId: data.client };
      }
      
      // Cas 3: createdBy existe
      const createdBy = data.createdBy?.id 
        ? { createdBy: { id: data.createdBy.id, ...data.createdBy } }
        : {};
 ...
 async creditExpediteur(expediteurId: string, montant: number, commandeId: string): Promise<void> {
  try {
    ...
    console.error("Erreur dans creditExpediteur:", error);
    throw error;
  }
}
