...
async createExchangeRequest(
  ...
  try {
    ...
    if (!commandeId || !productId || !description || !userId) {
    ...
    // Trouver le produit
    const product = commande.products?.find(p => 
    ...
    // Récupérer les détails complets du produit depuis Firestore
    const productDetails = await this.getProductById(product.productId || product.id);
    if (!productDetails) {
      throw new Error("Détails du produit non trouvés");
    }

    // Préparation des données avec vérification de chaque champ
    const exchangeData = {
      commandeId,
      productId: product.productId || product.id,
      productName: productDetails.name || 'Produit sans nom',
      ...
      productImage: productDetails.imageUrl || null,
      productDescription: productDetails.description || ''
    };

    // Validation finale des données avant envoi
    if (!exchangeData.productName) {
      throw new Error("Le nom du produit est requis");
    }

    console.log("Données d'échange à enregistrer:", exchangeData);
