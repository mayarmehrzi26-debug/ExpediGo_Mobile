...
async updateExchangeStatus(exchangeId: string, newStatus: string): Promise<void> {
  try {
    if (!exchangeId || !newStatus) {
      throw new Error("ID d'échange et nouveau statut sont requis");
    }

    const exchangeRef = doc(firebasestore, "exchanges", exchangeId);
    await updateDoc(exchangeRef, {
      status: newStatus,
      updatedAt: serverTimestamp()
    });
    
    console.log(`Statut de l'échange ${exchangeId} mis à jour à: ${newStatus}`);
  } catch (error) {
