import { addDoc, collection, doc, getDoc, getDocs, increment, limit, orderBy, query, updateDoc, where,writeBatch, serverTimestamp  } from "firebase/firestore";
...
export class LivraisonModel {
  ...
  async creditExpediteur(expediteurId: string, montant: number, commandeId: string): Promise<void> {
    try {
      console.log(`Tentative crédit: ${expediteurId}, ${montant}€`);
