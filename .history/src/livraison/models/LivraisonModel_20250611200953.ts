...
async createExchangeRequest(
  ...
  try {
    ...
    const product = commande.products?.find(p => p.id === productId);
    if (!product) {
    ...
    const exchangeData = {
      ...
      description,
      status: "En attente", // ou "Échange demandé" selon votre workflow
      requestedBy: userId,
    ...
    // Ajouter à la collection "exchanges"
    await addDoc(collection(firebasestore, "exchanges"), exchangeData);

    // Mettre à jour le statut de la commande (optionnel)
    await updateDoc(doc(firebasestore, "livraisons", commandeId), {
