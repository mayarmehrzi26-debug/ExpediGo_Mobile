import { addDoc,setDoc,collection, doc, getDoc, getDocs, increment, limit, orderBy, query, serverTimestamp, updateDoc, where } from "firebase/firestore";
...
export class LivraisonModel {
  ...
  async creditExpediteur(expediteurId: string, montant: number, commandeId: string): Promise<void> {
    try {
      ...
      await addDoc(collection(firebasestore, "transactions"), {
        userId: expediteurId,
      ...
      console.error("Erreur simplifiée:", error);
      throw error;
    }
  }
