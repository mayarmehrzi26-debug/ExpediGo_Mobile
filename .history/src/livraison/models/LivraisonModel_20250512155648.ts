...
export class LivraisonModel {
  ...
  async creditExpediteur(expediteurId: string, montant: number, commandeId: string): Promise<void> {
    try {
      console.log('[DEBUG] Début creditExpediteur', { expediteurId, montant, commandeId });
  
      if (!expediteurId || !montant || montant <= 0) {
        console.error('[ERROR] Paramètres invalides');
        throw new Error("Données de crédit invalides");
      }
  
      const expediteurRef = doc(firebasestore, "users", expediteurId);
      const commandeRef = doc(firebasestore, "livraisons", commandeId);
      const soldeRef = doc(firebasestore, "solde", expediteurId);
  
      console.log('[DEBUG] Références créées');
  
      // Vérification que l'expéditeur existe
      const expediteurDoc = await getDoc(expediteurRef);
      if (!expediteurDoc.exists()) {
        console.error('[ERROR] Expéditeur non trouvé', expediteurId);
        throw new Error("Expéditeur non trouvé");
      }
      console.log('[DEBUG] Expéditeur trouvé', expediteurDoc.data());
  
      // Vérification que la commande existe
      const commandeDoc = await getDoc(commandeRef);
      if (!commandeDoc.exists()) {
        console.error('[ERROR] Commande non trouvée', commandeId);
        throw new Error("Commande non trouvée");
      }
      console.log('[DEBUG] Commande trouvée', commandeDoc.data());
  
      const batch = writeBatch(firebasestore);
  
      // 1. Vérifier si le solde existe déjà, sinon le créer
      ...
      if (!soldeDoc.exists()) {
        ...
        // 2. Mise à jour du solde en attente
      ...
      // 3. Mise à jour du statut de paiement de la commande
      ...
      batch.set(transactionRef, {
        userId: expediteurId,
        amount: montant,
        type: "livraison_credit",
        commandeId: commandeId,
        status: "en_attente", // Le solde est marqué comme non disponible initialement
        date: serverTimestamp(),
        description: `Crédit pour livraison ${commandeId}`
      });
  
      console.log('[DEBUG] Toutes les opérations préparées');
      await batch.commit();
  ...
  async getSolde(userId: string): Promise<{disponible: number, enAttente: number}> {
    try {
      const soldeDoc = await getDoc(doc(firebasestore, "solde", userId));
      if (!soldeDoc.exists()) {
  ...
  async transfererSoldeEnAttente(userId: string): Promise<void> {
    try {
      const soldeRef = doc(firebasestore, "solde", userId);
      const soldeDoc = await getDoc(soldeRef);
