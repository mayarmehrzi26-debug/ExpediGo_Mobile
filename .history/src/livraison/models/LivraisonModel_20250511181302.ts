...
export class LivraisonModel {
 ...
 match /users/{userId} {
  // Autoriser la lecture à l'utilisateur lui-même
  allow read: if request.auth != null && request.auth.uid == userId;
  
  // Autoriser spécifiquement les mises à jour du solde
  allow update: if request.auth != null && 
               request.auth.uid == userId &&
               request.resource.data.keys().hasOnly(['solde', 'lastUpdated']) &&
               request.resource.data.solde is number;
  
  // Règles existantes pour la création
  allow create: if request.resource.data.role in ["admin", "destinataire", "livreur"] &&
