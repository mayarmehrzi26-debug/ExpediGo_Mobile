...
export class LivraisonModel {
  ...
  async startTrackingLivraison(commandeId: string, livreurId: string) {
    try {
      const trackingRef = doc(firebasestore, "livraisonTracking", commandeId);
  ...
  async updateLivraisonPosition(commandeId: string, position: { latitude: number, longitude: number }) {
    try {
      const trackingRef = doc(firebasestore, "livraisonTracking", commandeId);
      await updateDoc(trackingRef, {
  ...
  async stopTrackingLivraison(commandeId: string) {
    try {
      const trackingRef = doc(firebasestore, "livraisonTracking", commandeId);
      await updateDoc(trackingRef, {
        isActive: false,
  ...
  async getLivraisonTracking(commandeId: string) {
    try {
      const docRef = doc(firebasestore, "livraisonTracking", commandeId);
      const docSnap = await getDoc(docRef);
      return docSnap.exists() ? docSnap.data() : null;
    } catch (error) {
  ...
  setupTrackingListener(commandeId: string, callback: (data: any) => void) {
    const docRef = doc(firebasestore, "livraisonTracking", commandeId);
