...
async createExchangeRequest(
  ...
  try {
    ...
    // Debug: Afficher les produits de la commande
    console.log("Produits dans la commande:", commande.products);

    let product;
    if (commande.products && Array.isArray(commande.products)) {
      product = commande.products.find(p => p.id === productId || p.productId === productId);
    }

    if (!product) {
      // Debug supplémentaire
      const productIds = commande.products?.map(p => p.id || p.productId) || [];
      console.log("IDs produits disponibles:", productIds);
      throw new Error(`Produit ${productId} non trouvé dans la commande. Produits disponibles: ${productIds.join(', ')}`);
    }
