...
export class LivraisonModel {
  ...
  async createConversation(participants: string[]): Promise<string> {
    try {
      ...
      // Vérification que les deux participants existent
      const user1 = await this.getUserById(participants[0]);
      const user2 = await this.getUserById(participants[1]);
      
      if (!user1 || !user2) {
        throw new Error(`Un ou plusieurs participants n'existent pas (${!user1 ? participants[0] : ''} ${!user2 ? participants[1] : ''})`);
      }
      ...
      const conversationData = {
        ...
        status: "active",
        participantNames: {
          [participants[0]]: user1.displayName || user1.name || "Utilisateur 1",
          [participants[1]]: user2.displayName || user2.name || "Utilisateur 2"
        }
      };
  ...
  async findExistingConversation(participants: string[]): Promise<string | null> {
    try {
      ...
      if (!user1Exists || !user2Exists) {
        console.log(`Un ou plusieurs participants n'existent pas (${user1Exists ? 'user1 existe' : 'user1 manquant'}, ${user2Exists ? 'user2 existe' : 'user2 manquant'})`);
        return null;
      }
