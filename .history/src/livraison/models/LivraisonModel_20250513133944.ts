...
export class LivraisonModel {
    ...
    try {
      ...
      return {
        ...
        createdBy: data.createdBy ? {
          id: data.createdBy.id || data.createdBy.uid,
          name: data.createdBy.name || data.createdBy.displayName || 'Expéditeur inconnu'
        } : null,
        ...data,
  ...
  async creditExpediteur(expediteurId: string, montant: number, commandeId: string): Promise<void> {
    try {
      console.log(`Tentative de crédit: ${montant}€ pour l'expéditeur ${expediteurId}`);
  
      // Validation renforcée
      if (!expediteurId) throw new Error("ID expéditeur requis");
      if (typeof montant !== 'number' || montant <= 0) {
        console.warn(`Montant non crédité (${montant}€)`);
        return; // On sort sans erreur si montant <= 0
      }
  
      const batch = writeBatch(firebasestore);
      const soldeRef = doc(firebasestore, "soldes", expediteurId);
      
      // Vérifier que l'expéditeur existe bien dans la collection users
      const expediteurDoc = await getDoc(doc(firebasestore, "users", expediteurId));
      if (!expediteurDoc.exists()) {
        throw new Error(`L'expéditeur ${expediteurId} n'existe pas dans la collection users`);
      }
      ...
      await batch.commit();
      console.log(`Crédit de ${montant}€ effectué avec succès pour ${expediteurId}`);
  
    } catch (error) {
      console.error("Échec du crédit:", {
        error: error.message,
        expediteurId,
        montant,
        commandeId
