...
export class LivraisonModel {
  ...
  async getExpediteursForCommandes(livraisons: any[]): Promise<Map<string, string>> {
    const expediteurIds = [...new Set(livraisons.map(l => l.createdBy))];
    const expediteursMap = new Map<string, string>();
  
    if (expediteurIds.length === 0) return expediteursMap;
  
    const usersQuery = query(
      collection(firebasestore, "users"),
      where("uid", "in", expediteurIds)
    );
  
    const usersSnapshot = await getDocs(usersQuery);
    usersSnapshot.forEach(doc => {
      expediteursMap.set(doc.id, doc.data().name);
    });
  
    return expediteursMap;
  }
