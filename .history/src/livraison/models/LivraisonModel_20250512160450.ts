...
export class LivraisonModel {
  ...
  async creditExpediteur(expediteurId: string, montant: number, commandeId: string): Promise<void> {
    try {
      ...
      const transactionsRef = collection(firebasestore, "transactions");
  
      // Vérifications
      const [expediteurDoc, commandeDoc] = await Promise.all([
      ...
      // Batch pour les opérations atomiques
      ...
      // 2. Mise à jour du statut de paiement de la commande
      ...
      // Exécution du batch
      await batch.commit();
      console.log('[SUCCESS] Crédit effectué avec succès');
