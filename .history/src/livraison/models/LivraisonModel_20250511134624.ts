...
export class LivraisonModel {
  ...
  async creditExpediteur(expediteurId: string, montant: number): Promise<void> {
    try {
      if (!expediteurId || !montant || montant <= 0) {
        throw new Error("Données invalides pour le crédit");
      }
      ...
      // Ajouter une entrée dans l'historique des transactions
      await addDoc(collection(firebasestore, "transactions"), {
        ...
        date: new Date(),
        status: "completed",
        description: `Crédit pour livraison avec paiement à percevoir`
      });
  
    } catch (error) {
      console.error("Erreur lors du crédit de l'expéditeur:", error);
      throw error;
    }
  }
}