...
// Méthode helper pour l'envoi d'email avec retry
...
// Version améliorée de sendDeliveryEmail
private async sendDeliveryEmail(
  ...
  try {
    ...
    const apiUrl = process.env.EMAIL_API_URL || 'http://localhost:3001/send-delivery-email';
...
private async sendDeliveryEmail(email: string, deliveryId: string, qrCodeUrl: string): Promise<void> {
  try {
    const response = await fetch('http://localhost:3001/send-delivery-email', {
      method: 'POST',
      headers: {
