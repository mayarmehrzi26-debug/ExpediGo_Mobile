...
async fetchInitialData() {
  try {
    const [products, clients, adresses, defaultStatus] = await Promise.all([
      ...
      this.model.fetchClients(), // Cette méthode est maintenant filtrée
...
async saveDelivery(
  ...
  if (!selectedAddress || !selectedClient || selectedProducts.length === 0 || !selectedPayment) {
    return { 
      success: false, 
...
private async sendDeliveryEmail(
  ...
  try {
    ...
    const apiUrl = 'http://192.168.117.160:3001/send-delivery-email'; // Remplacez X par votre valeur
