...
useEffect(() => {
  const fetchDeliveryData = async () => {
    try {
      ...
      if (deliveryData) {
        // Vérifier si products existe et est un tableau
        const productsArray = deliveryData.products || [];
        
        // Récupérer les informations des produits seulement si le tableau n'est pas vide
        const productsData = productsArray.length > 0 
          ? await Promise.all(
              productsArray.map(async (product: any) => {
                try {
                  const productDoc = await getDoc(doc(firebasestore, "products", product.productId));
                  return {
                  ...
                  console.error(`Erreur lors de la récupération du produit ${product.productId}:`, error);
                  return {
          ...
          : [{ name: "Aucun produit spécifié", quantity: 0, price: 0 }];

        const [clientDoc, addressDoc] = await Promise.all([
        ...
        setDelivery({
          ...
          destination: clientDoc?.data()?.address || "Adresse inconnue",
          products: productsData,
          origine: addressDoc?.data()?.address || "Adresse inconnue",
  ...
  return (
    <View style={[styles.deliveryCard, isSelected && styles.selectedCard]}>
      ...
      <View style={styles.infoRow}>
        ...
        <Text style={styles.infoValue}>{delivery.origine}</Text>
      </View>
      <View style={styles.infoRow}>
