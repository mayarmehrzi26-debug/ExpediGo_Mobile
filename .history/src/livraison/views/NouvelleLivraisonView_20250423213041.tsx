...
import { RootState } from "../redux/store"; // Importez RootState
const NouvelleLivraisonScreen: React.FC = () => {
  const navigation = useNavigation();
  const dispatch = useDispatch();
  
  // Utilisez les sélecteurs individuels pour une meilleure performance
  const adresses = useSelector((state: RootState) => state.livraison.adresses);
  const clients = useSelector((state: RootState) => state.livraison.clients);
  const produits = useSelector((state: RootState) => state.livraison.produits);
  const paiementOptions = useSelector((state: RootState) => state.livraison.paiementOptions);
  const selectedAddress = useSelector((state: RootState) => state.livraison.selectedAddress);
  const selectedClient = useSelector((state: RootState) => state.livraison.selectedClient);
  const selectedProduct = useSelector((state: RootState) => state.livraison.selectedProduct);
  const selectedPayment = useSelector((state: RootState) => state.livraison.selectedPayment);
  const quantity = useSelector((state: RootState) => state.livraison.quantity);
  const isExchange = useSelector((state: RootState) => state.livraison.isExchange);
  const isFragile = useSelector((state: RootState) => state.livraison.isFragile);
  const termsAccepted = useSelector((state: RootState) => state.livraison.termsAccepted);
  const totalAmount = useSelector((state: RootState) => state.livraison.totalAmount);
  ...
  // Fonction pour gérer la sélection des produits et calculer le total
  const handleProductSelect = (value: string) => {
    dispatch(setSelectedProduct(value));
    const product = produits.find(p => p.value === value);
    if (product) {
      dispatch(setQuantity(1)); // Réinitialise la quantité à 1
      // Calcul automatique du montant total
      const newTotal = product.price * quantity;
      // Vous devrez ajouter une action setTotalAmount dans votre slice
    }
  };
