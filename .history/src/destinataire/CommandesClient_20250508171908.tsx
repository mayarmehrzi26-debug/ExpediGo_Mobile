...
import { LivraisonModel } from "../livraison/rmodels/Livraison";
...
const CommandesClient: React.FC = () => {
  ...
  const livraisonModel = new LivraisonModel();
  ...
  useEffect(() => {
    const fetchCommandes = async () => {
      try {
        ...
        // Récupération des données initiales via le modèle
        const { clients, adresses } = await livraisonModel.fetchInitialData();
        const adressesMap = new Map(adresses.map(addr => [addr.value, addr.label]));
        
        // Trouver le client correspondant à l'utilisateur
        const client = clients.find(c => c.createdBy === currentUser.uid);
        if (!client) {
        ...
        for (const doc of commandesSnapshot.docs) {
          ...
          if (data.status !== "Supprimée") {
            commandesList.push({
              id: doc.id,
              origin: adressesMap.get(data.address) || "Adresse inconnue",
