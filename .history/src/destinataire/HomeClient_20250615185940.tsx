...
import { arrayUnion, collection, doc, getDoc, getDocs, orderBy, query, serverTimestamp, updateDoc, where } from 'firebase/firestore';
...
import BarcodeScanner from './components/BarcodeScannerCL';
...
const HomeClient: React.FC<HomeScreenProps> = ({ navigation }) => {
  ...
  const fetchNotifications = async () => {
    try {
       const user = firebaseAuth.currentUser;
    if (user) {
      // Récupérer les notifications globales
      ...
      const globalNotifications = globalSnapshot.docs.map(doc => ({
        id: `global_${doc.id}`, // ← Ajoutez un préfixe
        source: 'global',
      ...
      const userNotifications = userSnapshot.docs.map(doc => ({
        id: `user_${doc.id}`, // ← Ajoutez un préfixe
        source: 'user',
          title: doc.data().title,
          body: doc.data().body,
          data: doc.data().data,
        ...
        const uniqueNotifications = [...globalNotifications, ...userNotifications]
        .filter((notif, index, self) => 
          index === self.findIndex(n => (
            n.data?.trackingId === notif.data?.trackingId || // Si vous avez un ID commun
            n.id === notif.id // Sinon, comparez les IDs avec préfixe
          ))
        );

      setNotificationsList(uniqueNotifications);
    }
  } catch (error) {
...
const markAsRead = async (notificationId: string) => {
  try {
    ...
    // Détecter si c'est une notification globale ou utilisateur
    const isGlobal = notificationId.startsWith('global_');
    const cleanId = isGlobal ? notificationId.replace('global_', '') : notificationId;

    if (isGlobal) {
      // Notification globale - mettre à jour dans 'notifications'
      await updateDoc(
        doc(firebasestore, 'notifications', cleanId),
        { readBy: arrayUnion(user.uid) }
      );
    } else {
      // Notification utilisateur - mettre à jour dans 'users/{uid}/notifications'
      await updateDoc(
        doc(firebasestore, `users/${user.uid}/notifications`, cleanId),
        { read: true }
      );
    }
    ...
    setUnreadCount(prev => Math.max(0, prev - 1)); // Empêcher les valeurs négatives
  } catch (error) {
    console.error('Error marking notification as read:', error);
    if (error.code === 'not-found') {
      // Supprimer la notification manquante de l'état local
  ...
  const searchOrderByTrackingCode = async () => {
    ...
    try {
      ...
      if (orderDoc.exists()) {
        navigation.navigate('CommandeDetailsClient', { commandeId: trackingCode });
      } else {
