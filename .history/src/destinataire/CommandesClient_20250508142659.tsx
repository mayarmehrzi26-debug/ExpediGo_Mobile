...
const CommandesClient: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCommandes = async () => {
      try {
        ...
        // Méthode 1: Si vous avez une collection users avec clientId
        ...
        if (!userSnapshot.empty) {
          ...
          // Méthode 2: Si client a directement le uid
  ...
  for (const doc of commandesSnapshot.docs) {
    ...
    if (data.status !== "Supprimée") {
      // Récupérer le nom de l'adresse d'origine
      let originName = "Adresse inconnue";
      if (data.origin) {
        const originDoc = await getDoc(doc(firebasestore, "adresses", data.origin));
        if (originDoc.exists()) {
          originName = originDoc.data().nom || "Adresse inconnue"; // Supposons que le champ s'appelle "nom"
        }
      }

      // Récupérer le nom de l'adresse de destination
      let destinationName = "Adresse inconnue";
      if (data.destination) {
        const destinationDoc = await getDoc(doc(firebasestore, "adresses", data.destination));
        if (destinationDoc.exists()) {
          destinationName = destinationDoc.data().nom || "Adresse inconnue";
        }
      }
