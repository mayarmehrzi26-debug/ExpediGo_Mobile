...
interface CommandeDetailsData {
  ...
  origin: string;
  destination: string;
  expediteurName: string;  // Changé de clientName à expediteurName
  expediteurPhone: string; // Changé de clientPhone à expediteurPhone
  status: string;
  payment: string;
  isFragile: boolean;
  createdAt: Date;
  ...
  livreur: string;
  notes: string;
  productName: string;     // Nouveau champ
...
const CommandeDetails: React.FC<CommandeDetailsProps> = ({ route, navigation }) => {
    ...
    const fetchCommandeDetails = async () => {
      try {
        ...
        setCommande({
          ...
          livreurPhone: commandeDetails.livreurPhone || '', // Ajout du téléphone du livreur
  ...
  const handleMessageLivreur = async () => {
    try {
      ...
      // Vérifiez que le livreur est assigné
      ...
      <Text style={styles.productDescription}>{commande.productDescription}</Text>
          ...
          <View style={styles.detailsContainer}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{commande.expediteurPhone}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  productSection: {
    ...
    marginBottom: 16,
    flexDirection: 'row',
    justifyContent: 'space-between', // ✅ Aligne à gauche/droite
    alignItems: 'center',            // ✅ Centre verticalement (optionnel)
  },
  productInfo: {
    marginTop: 0,                     // ✅ Plus de décalage vertical
    alignItems: 'flex-end',          // ✅ Aligne le texte à droite dans ce bloc
  },
productNameText: {
  fontSize: 16,
  fontWeight: '600',
