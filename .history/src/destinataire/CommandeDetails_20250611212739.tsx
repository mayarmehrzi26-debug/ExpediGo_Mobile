   ...
   let products = [];
    if (Array.isArray(commandeDoc.products)) {
      products = await Promise.all(
        commandeDoc.products.map(async (p) => {
...
const handleSubmitExchange = async () => {
  if (!selectedProductId || !exchangeDescription || !commande || !currentUser) {
    alert('Veuillez sélectionner un produit et fournir une description');
    return;
  }

  // Vérification supplémentaire
  const productExists = commande.products?.some(
    p => p.id === selectedProductId || p.productId === selectedProductId
  );
  
  if (!productExists) {
    alert('Le produit sélectionné n\'existe pas dans cette commande');
    return;
  }
