...
const markAsRead = async (notificationId: string, source: 'global' | 'user') => {
  try {
    const user = firebaseAuth.currentUser;
    if (!user) return;
    ...
    if (source === 'global') {
      await updateDoc(
        doc(firebasestore, 'notifications', notificationId.replace('global_', '')),
        { readBy: arrayUnion(user.uid) }
      );
    } else {
      await updateDoc(
        doc(firebasestore, `users/${user.uid}/notifications`, notificationId.replace('user_', '')),
        { read: true }
      );
    }
