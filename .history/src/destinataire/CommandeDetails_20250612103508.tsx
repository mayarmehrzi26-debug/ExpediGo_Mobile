...
const fetchExchangeDetails = async () => {
  try {
    const exchanges = await model.getExchangesForCommande(commandeId);
    if (exchanges.length > 0) {
      setExchangeDetails(exchanges[0]); // Prend le premier échange trouvé
    }
  } catch (error) {
    ...
    const fetchCommandeDetails = async () => {
      try {
        ...
        setCommande({
          ...
          createdAt: commandeDetails.createdAt ? new Date(commandeDetails.createdAt) : new Date(),
          ...
          products: commandeDetails.products || []
        });
      } catch (err) {
 ...
 const handleSubmitExchange = async () => {
    if (!selectedProductId || !exchangeDescription || !commande || !currentUser) {
    ...
    try {
      ...
      const selectedProduct = commande.products.find(
        p => p.id === selectedProductId || p.productId === selectedProductId
      );
      
      if (!selectedProduct) {
        alert('Produit non trouvé dans la commande');
        return;
      }

      await model.createExchangeRequest(
        commande.id,
        selectedProductId,
        exchangeDescription,
        currentUser.uid,
        selectedProduct.name // Passer le nom du produit explicitement
      );
      ...
      alert(`Échec de la demande d'échange: ${error.message}`);
    }
  };
      ...
      <ScrollView>
        <View style={styles.card}>
          ...
          <View style={styles.detailsContainer}>
            <Text style={styles.productsTitle}>Produits ({commande.products.length})</Text>
                ...
                <View style={styles.productInfo}>
                  ...
                  <View style={styles.productDetails}>
                    <View style={styles.detailBadge}>
                      <Text style={styles.detailBadgeText}>Qté: {product.quantity}</Text>
                    </View>
                    <View style={styles.detailBadge}>
                      <Text style={styles.detailBadgeText}>Prix: {product.price} DT</Text>
                    </View>
                  </View>
                </View>
          ...
          {/* Bouton de demande d'échange (visible seulement si le statut est "Livré") */}
          ...
          <View style={styles.modalContent}>
            {exchangeRequestSent ? (
              <View style={styles.successContainer}>
                ...
                <Text style={styles.successText}>Demande d'échange envoyée avec succès</Text>
                <TouchableOpacity
              ...
              <>
                <Text style={styles.modalTitle}>Demande d'échange</Text>
                
                <Text style={styles.modalSubtitle}>Sélectionnez le produit à échanger:</Text>
  ...
  <Pressable
    ...
    style={[
      ...
      selectedProductId === (product.id || product.productId) && styles.selectedProductOption
    ]}
    onPress={() => setSelectedProductId(product.id || product.productId)}
  >
    <Text style={styles.productOptionText}>{product.name}</Text>
    {selectedProductId === (product.id || product.productId) && (
      <Icon name="check" size={20} color="#44076a" />
    )}
  </Pressable>
))}
{commande.status === "Echange" && (
  <>
    <TouchableOpacity 
      style={styles.exchangeDetailsButton}
      onPress={() => {
        fetchExchangeDetails();
        setShowExchangeDetailsModal(true);
      }}
    >
          ...
          {exchangeDetails ? (
            <>
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Produit à échanger:</Text>
                <Text style={styles.detailValue}>{exchangeDetails.productName}</Text>
              </View>
              
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Description:</Text>
                <Text style={styles.detailValue}>{exchangeDetails.description}</Text>
              </View>
              
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Statut:</Text>
                <Text style={styles.detailValue}>{exchangeDetails.status}</Text>
              </View>
            ...
            <ActivityIndicator size="large" color="#44076a" />
                ...
                <Text style={styles.modalSubtitle}>Description de l'échange:</Text>
                <TextInput
                  style={styles.descriptionInput}
                  multiline
                  numberOfLines={4}
                  placeholder="Décrivez la raison de l'échange et les détails..."
                ...
                <View style={styles.modalButtons}>
                  ...
                  <TouchableOpacity
                    ...
                    disabled={!selectedProductId || !exchangeDescription}
                  >
