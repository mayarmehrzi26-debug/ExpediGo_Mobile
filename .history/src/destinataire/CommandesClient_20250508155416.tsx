    ...
    const fetchClientCommandes = async (clientId: string, clientData: any) => {
      ...
      for (const doc of commandesSnapshot.docs) {
        ...
        if (data.status !== "Supprimée") {
          ...
          if (data.origin) {
            try {
              const adresseDoc = await getDoc(doc(firebasestore, "adresses", data.origin));
              if (adresseDoc.exists()) {
                const adresseData = adresseDoc.data();
                // Utiliser directement le champ address qui contient l'adresse complète
                originAdresse = adresseData.address || "Adresse inconnue";
              }
            } catch (error) {
          ...
          // Récupérer l'adresse de destination (depuis clientData)
          let destinationAdresse = clientData.address || "Adresse inconnue";
