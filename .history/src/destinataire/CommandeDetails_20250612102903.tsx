...
const CommandeDetails: React.FC<CommandeDetailsProps> = ({ route, navigation }) => {
  ...
  const handleSubmitExchange = async () => {
    if (!selectedProductId || !exchangeDescription || !commande) {
      alert('Veuillez sélectionner un produit et fournir une description');
      return;
    }

    try {
      // Ici vous devriez implémenter l'appel à votre backend pour enregistrer la demande d'échange
      // Par exemple :
      // await model.requestExchange(commande.id, selectedProductId, exchangeDescription);
      
      // Simulation d'un envoi réussi
      console.log('Demande d\'échange envoyée pour le produit:', selectedProductId);
      console.log('Description:', exchangeDescription);
      ...
      alert('Une erreur est survenue lors de la demande d\'échange');
    }
  };
                  ...
                  {commande.products.map((product, index) => (
                    <Pressable
                      ...
                      style={[
                        ...
                        selectedProductId === product.id && styles.selectedProductOption
                      ]}
