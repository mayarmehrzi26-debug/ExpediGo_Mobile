  ...
  useEffect(() => {
    ...
    const fetchClientCommandes = async (clientId: string, clientData: any) => {
      ...
      adressesSnapshot.forEach(doc => {
        adressesMap.set(doc.id, doc.data().nom); // Supposons que le champ s'appelle "nom"
      });
      ...
      for (const doc of commandesSnapshot.docs) {
        ...
        if (data.status !== "Supprimée") {
          ...
          const originAdresse = data.origin ? adressesMap.get(data.origin) || "Adresse inconnue" : "Adresse inconnue";
          
          // Récupérer le nom de l'adresse de destination
          const destinationAdresse = data.destination ? adressesMap.get(data.destination) || "Adresse inconnue" : "Adresse inconnue";
    
          commandesList.push({
            id: doc.id,
            origin: originAdresse,
            destination: clientData.address,
