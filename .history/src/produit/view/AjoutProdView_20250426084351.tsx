...
import { AjoutProdPresenter } from "../presenter/AjoutProdPresenter";

interface AjoutProdProps {
  navigation: any;
}

const AjoutProdView: React.FC<AjoutProdProps> = ({ navigation }) => {
  const [productName, setProductName] = useState("");
  const [amount, setAmount] = useState("");
  const [description, setDescription] = useState("");
  const [image, setImage] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [presenter] = useState(new AjoutProdPresenter({
    setImage: (uri) => setImage(uri),
    setLoading: (isLoading) => setLoading(isLoading),
    showPermissionError: () => Alert.alert(
    ...
    onSubmitSuccess: () => Alert.alert("Succès", "Produit ajouté avec succès", [
  ...
  const pickImage = async () => {
    await presenter.pickImage();
  };

  const handleSubmit = async () => {
    await presenter.submitProduct(productName, amount, description, image);
  };
        ...
        <View style={styles.content}>
          ...
          <View style={styles.formGroup}>
            ...
            <Text style={styles.helperText}>
              Veuillez saisir le montant en dinars tunisiens, par exemple
